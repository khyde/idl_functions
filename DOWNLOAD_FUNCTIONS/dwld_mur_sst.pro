; $ID:	DWLD_MUR_SST.PRO,	2023-09-21-13,	USER-KJWH	$
  PRO DWLD_MUR_SST, YEARS=YEARS, VERSION=VERSION, DATERANGE=DATERANGE, LOGLUN=LOGLUN, LIMIT=LIMIT, RECENT=RECENT, RYEARS=RYEARS, CHECK_FILES=CHECK_FILES
...
;
; PURPOSE:
;   Download the MUR SST (global 1km) data from the NASA PoDAAC
;
; CATEGORY:
;   DOWNLOAD_FUNCTIONS
;
...
;
; OUTPUTS:
;   New files downloaded into !S.MUR/L4/NC/SST
;   
; OPTIONAL OUTPUTS:
;   None
;
...
;
; AUTHOR:
;   This program was written on May 17, 2021 by Kimberly J. W. Hyde, Northeast Fisheries Science Center | NOAA Fisheries | U.S. Department of Commerce, 28 Tarzwell Dr, Narragansett, RI 02882
;    
; MODIFICATION HISTORY:
;   May 17, 2021 - KJWH: Initial code written
;   Nov 18, 2021 - KJWH: Now downloading files from the PoDAAC cloud instead of their data servers
;                        Updated the directory information
;                        Now saving the replaced CHECKSUM files into and ARCHIVE folder
;   Feb 28, 2022 - KJWH: Changed the structure tag SERVER_FILES to REMOTE_FILES
;-
; ****************************************************************************************************
  ROUTINE_NAME = 'DWLD_MUR_SST'
  COMPILE_OPT IDL2
  SL = PATH_SEP()
    
  CURRENT_VERSION = 'VERSION_4.1'
  USER=' --user=khyde --password=NASAPhyt0pl@nkt0nRS';FydOA4zodKWghvNRKH2P'
  IF HAS(!S.COMPUTER,'NECLNAMAC') THEN WCMD = '/usr/local/bin/wget' ELSE WCMD = 'wget'


  DP = DATE_PARSE(DATE_NOW())
  LOGDIR =  !S.LOGS + 'IDL_DOWNLOADS' + SL + 'MUR' + SL & DIR_TEST, LOGDIR
  LOGFILE = LOGDIR +  'MUR-SST' + '_' + DATE_NOW(/DATE_ONLY) + '.log'
  ...
  PLUN,LUN,'Downloading MUR files... ', 0
  
  IF N_ELEMENTS(VERSION) EQ 0 THEN VERSION = '4.1'
  IF N_ELEMENTS(YEARS)   EQ 0 THEN YRS = YEAR_RANGE('2002',DP.YEAR,/STRING) ELSE YRS = STRING(YEARS)
  ...
  DIR = !S.MUR_SOURCE + 'V' + VERSION + SL + 'SOURCE' + SL 
  NDIR = DIR + 'MAPPED_1KM_DAILY' + SL +'SST' + SL
  ...
  CASE VERSION OF
    '4.1': BEGIN
      ...
      CLOUD = 'https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-protected/MUR-JPL-L4-GLOB-v4.1/'
      PUBLIC = 'https://archive.podaac.earthdata.nasa.gov/podaac-ops-cumulus-public/MUR-JPL-L4-GLOB-v4.1/'
    END  
  ENDCASE

  SDATES = SENSOR_DATES('MUR')                                                        ; Get the SENSOR daterange
...
;  URLS   = URLS[YSUBS]      
  ...
  CMD = WCMD + ' --tries=3 --retry-connrefused -c -N' + USER + ' -i ' + DIR + 'CHECKSUMS_TO_DOWNLOAD.txt -a ' + LOGFILE        ; Checksum download command
  PLUN, LUN, CMD
  CD, CDIR
  SPAWN, CMD, LOG, ERR
