...
; ****************************************************************************************************
  ...
  FOR T=0, N_ELEMENTS(TYPES)-1 DO BEGIN
    ...
    CASE TYP OF 
      'RAN': BEGIN & DDIR = !S.DATASETS_SOURCE + 'ACSPO/' & DTR = DTRS & IF DATE_2JD(DTR[0]) LT DATE_2JD(SENDATES[0]) THEN DTR[0] = SENDATES[0] & END
      'NRT': BEGIN & DDIR = !S.DATASETS_SOURCE + 'ACSPONRT/' & IF ~N_ELEMENTS(DATERANGE) THEN DTR = NRTDATES ELSE DTR=DTRS & END
    ENDCASE  
    
    DIR = DDIR + VER + SL + 'SOURCE' + SL
