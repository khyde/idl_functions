; $ID:	DWLD_ESA_OCCCI.PRO,	2023-09-21-13,	USER-KJWH	$
  PRO DWLD_ESA_OCCCI, YEARS=YEARS, PRODS=PRODS, VERSION=VERSION, LOGLUN=LOGLUN, LIMIT=LIMIT, RECENT=RECENT, RYEARS=RYEARS, MONTHLY=MONTHLY, CHECK_FILES=CHECK_FILES
...
; ****************************************************************************************************
  ...
  FOR R=0, N_ELEMENTS(PRODS)-1 DO BEGIN
    ...
    FOR N=0, N_ELEMENTS(YRS)-1 DO BEGIN
      YR = YRS[N]
      DIR = !S.OCCCI + DIRVER + SL +'SOURCE' + SL + 'NC' + SL + DR +SL & DIR_TEST, DIR
      CD, DIR
