...
; ===> SPECIAL MAPPING CASES
  ...
  CASE [1] OF
    ...
    HAS(MAP_IN,'L3B'): BEGIN
      IF MAP_OUT EQ 'CHES_BAY' THEN BEGIN
        TMPMAP = MAPS_L3B_2MAP(ARRAY,BINS,MAP_IN=L3B,MAP_OUT='NES',LONS=SUBSET_LONS,LATS=SUBSET_LATS,INIT=INIT, STRUCT_XPYP= STRUCT_XPYP)
        RETURN, MAPS_REMAP(TMPMAP,MAP_IN='NES',MAP_OUT='CHES_BAY',INIT=INIT,STRUCT_XPYP=STRUCT_XPYP)
      ENDIF
      RETURN, MAPS_L3B_2MAP(ARRAY,BINS,MAP_IN=L3B,MAP_OUT=MAP_OUT,LONS=SUBSET_LONS,LATS=SUBSET_LATS,INIT=INIT, STRUCT_XPYP= STRUCT_XPYP)
    END
