...
;	===> If PX and PY not provided get default sizes for this map
  ...
  IF COUNT EQ 1 THEN BEGIN
    ...
    IF STRUCT_HAS(STR,'LONS') THEN LONS=STR.LONS ELSE LONS=STR.LON
    IF STRUCT_HAS(STR,'LATS') THEN LATS=STR.LATS ELSE LATS=STR.LAT
    RETURN, STR
  ENDIF  
...
;===> If MAPS_2LONLAT-MAP file already exists then read and return
  IF FILE_TEST(OUTFILE) EQ 1 AND ~KEY(OVERWRITE) AND GET_MTIME(OUTFILE,/JD) GT MTIME THEN BEGIN
    ...
    IF STRUCT_HAS(D,'LONS') THEN LONS=D.LONS ELSE LONS=D.LON
    IF STRUCT_HAS(D,'LATS') THEN LATS=D.LATS ELSE LATS=D.LAT
