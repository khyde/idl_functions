...
; ****************************************************************************************************
  ...
  ;  AZIMUTH = MAPS_AZIMUTH(LONS=CN_LONS,LATS=CN_LATS)
    ...
    ; Check max lats (above calculations can return values slightly greater than 90 (e.g. 90.000000004))
    OK = WHERE(CT_EDGE GT 90.0, COUNT_90)
    IF COUNT_90 GT 0 THEN BEGIN
      IF MAX(CT_EDGE[OK] GT 90.1) THEN MESSAGE, 'ERROR: Latitude exceeds 90'
      CT_EDGE[OK] = 90.0 
    ENDIF  
