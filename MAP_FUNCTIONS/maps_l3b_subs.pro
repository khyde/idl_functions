...
; *****************************************************************************************************************
  ...
  FOR N=0, N_ELEMENTS(DATASETS)-1 DO BEGIN
    ...
    F = FILE_SEARCH(!S.IDL_DEMO_FILES + '*' + DATASET + '*.nc',COUNT=COUNT)
    IF COUNT EQ 0 THEN MESSAGE, 'ERROR: NEED SAMPLE FILE TO MAKE THE MASTER L3B FILES'
    D = READ_NC(F[0],PRODS=['LON','LAT','LATITUDE','LONGITUDE'])
    IF HAS_TAG(SD,'LON') THEN LON = D.SD.LON.IMAGE ELSE LON = D.SD.LONGITUDE.IMAGE
    IF HAS_TAG(SD,'LAT') THEN LAT = D.SD.LAT.IMAGE ELSE LAT = D.SD.LATITUDE.IMAGE
