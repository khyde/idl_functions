...
PRO PRODS_2PNG, $
  ...
  ; ===> ADD POLYGON
  ADD_POLYGON=ADD_POLYGON,POLYGON_LONS=POLYGON_LONS,POLYGON_LATS=POLYGON_LATS,POLYGON_LINESTYLE=POLYGON_LINESTYLE,POLYGON_COLOR=POLYGON_COLOR,POLYGON_FILL_BACKGROUND=POLYGON_FILL_BACKGROUND,POLYGON_FILL_COLOR=POLYGON_FILL_COLOR,POLYGON_TRANSPARENCY=POLYGON_TRANSPARENCY,POLYGON_THICK=POLYGON_THICK,$
  
  ; ===> ADD POLYLINE
  ADD_POLYLINE=ADD_POLYLINE,POLYLINE_LONS=POLYLINE_LONS,POLYLINE_LATS=POLYLINE_LATS,POLYLINE_LINESTYLE=POLYLINE_LINESTYLE,POLYLINE_COLOR=POLYLINE_COLOR,POLYLINE_TRANSPARENCY=POLYLINE_TRANSPARENCY,POLYLINE_THICK=POLYLINE_THICK,$
...
;-
  ...
  FOR NTH=0, N_ELEMENTS(FILES)-1 DO BEGIN
    ...
    FPP = PARSE_IT(_PNGFILE)
    IF ~KEYWORD_SET(NO_SAVE) THEN DIR_TEST, FPP.DIR
...
;TODO Update the contouring functions      
    ...
    IF KEY(ADD_LONLAT) OR ANY(LONS) OR ANY(LONNAMES) OR KEY(ADD_LONLAT_LABELS) THEN BEGIN
      ...
      IF HAS(LL,'LATS') THEN LLLATS = LL.LATS ELSE LLLATS = LL.LAT 
      IF HAS(LL,'LONS') THEN LLLONS = LL.LONS ELSE LLLONS = LL.LON
      IF N_ELEMENTS(_LATS) GE 1 THEN BEGIN
        FIN = WHERE(FINITE(LLLATS) EQ 0,COUNT_FIN) & IF COUNT_FIN GE 1 THEN LLLATS[FIN] = MISSINGS(0.0)
        ZWIN, LLLATS
        CONTOUR,LLLATS,LEVELS=_LATS,MIN_VALUE=-90,MAX_VALUE=90,C_CHARSIZE=LL_SIZE,C_LABELS=_LATNAMES,C_THICK=LL_THICK, $
      ...
      IF N_ELEMENTS(_LONS) GE 1 THEN BEGIN
        FIN = WHERE(FINITE(LLLONS) EQ 0,COUNT_FIN) & IF COUNT_FIN GE 1 THEN LLLONS[FIN] = MISSINGS(0.0)
        ZWIN, LLLONS
        CONTOUR,LLLONS,LEVELS=_LONS,MIN_VALUE=-180,MAX_VALUE=180,C_CHARSIZE=LL_SIZE,C_LABELS=_LONNAMES,C_THICK=LL_THICK, $$
          XSTYLE=5,YSTYLE=5,XMARGIN=[0,0],YMARGIN=[0,0],POSITION=[0,0,1,1], CLOSED=0,/NOERASE,_EXTRA=_extra
        LL_LONS = TVRD()
        ZWIN
    ...
    IF KEYWORD_SET(ADD_POINTS) THEN BEGIN
      IF IDLTYPE(ADD_POINTS) EQ 'STRUCT' THEN BEGIN
        FOR SY=0, N_TAGS(ADD_POINTS)-1 DO BEGIN
          ...
          ZWIN
          PSY = SYMBOL(PX,PY,SYMBOL=STR.SYMBOL,/DEVICE, SYM_FILLED=STR.SYM_FILLED, SYM_COLOR=STR.SYM_COLOR, SYM_FILL_COLOR=STR.SYM_FILL_COLOR, SYM_SIZE=STR.SYM_SIZE, SYM_THICK=STR.SYM_THICK, TARGET=IMG)
        ENDFOR
      ENDIF ELSE BEGIN
        MAPS_SET, AMAP
        LL = MAP_DEG2IMAGE(MAPS_BLANK(AMAP),POINT_LONS,POINT_LATS,X=PX,Y=PY)
        ZWIN
        PSY = SYMBOL(PX,PY,SYMBOL=POINT_SYMBOL,/DEVICE, SYM_FILLED=POINT_SYM_FILL, SYM_COLOR=POINT_COLOR, SYM_FILL_COLOR=POINT_FILL_COLOR, SYM_SIZE=POINT_SIZE, TARGET=IMG)
      ENDELSE
    ENDIF
    
    ;######### ADD POLYGON #####################
    IF KEYWORD_SET(ADD_POLYGON) THEN BEGIN
      IF IDLTYPE(ADD_POLYGON) EQ 'STRUCT' THEN BEGIN
        FOR SY=0, N_TAGS(ADD_POLYGON)-1 DO BEGIN
          STR = ADD_POLYGON.(SY)
          MAPS_SET, AMAP, PX=IMG_DIMS[0], PY=IMG_DIMS[1]
          LL = MAP_DEG2IMAGE(MAPS_BLANK(AMAP,PLX=IMG_DIMS[0],PLY=IMG_DIMS[1]),STR.LONS,STR.LATS,X=PX,Y=PY)
          ZWIN
          PLX = PLX[WHERE(PLX GT -1)] & PLY = PLY[WHERE(PLY GT -1)]
          PSY = POLYGON(PLX,PLY,LINESTYLE=STR.POLYGON_LINESTYLE,/DEVICE,COLOR=STR.POLYGON_COLOR,FILL_BACKGROUND=STR.POLYGON_FILL_BACKGROUND,FILL_COLOR=STR.POLYGON_FILL_COLOR,FILL_TRANSPARENCY=STR.POLYGON_TRANSPARENCY, THICK=STR.POLYGON_THICK, TARGET=IMG)                  
        ENDFOR
      ENDIF ELSE BEGIN
        MAPS_SET, AMAP
        LL = MAP_DEG2IMAGE(MAPS_BLANK(AMAP),POLYGON_LONS,POLYGON_LATS,X=PLX,Y=PLY)
        PLX = PLX[WHERE(PLX GT -1)] & PLY = PLY[WHERE(PLY GT -1)]
        ZWIN
        PSY = POLYGON(PLX,PLY,LINESTYLE=POLYGON_LINESTYLE,/DEVICE,COLOR=POLYGON_COLOR,FILL_BACKGROUND=POLYGON_FILL_BACKGROUND,FILL_COLOR=POLYGON_FILL_COLOR,FILL_TRANSPARENCY=POLYGON_TRANSPARENCY, THICK=POLYGON_THICK, TARGET=IMG)
      ENDELSE
    ENDIF
    
    ;######### ADD POLYLINE #####################
    IF KEYWORD_SET(ADD_POLYLINE) THEN BEGIN
      IF IDLTYPE(ADD_POLYLINE) EQ 'STRUCT' THEN BEGIN
        FOR SY=0, N_TAGS(ADD_POLYLINE)-1 DO BEGIN
          STR = ADD_POLYLINE.(SY)
          MAPS_SET, AMAP, PX=IMG_DIMS[0], PY=IMG_DIMS[1]
          LL = MAP_DEG2IMAGE(MAPS_BLANK(AMAP,PGX=IMG_DIMS[0],PGY=IMG_DIMS[1]),STR.LONS,STR.LATS,X=PX,Y=PY)
          ZWIN
          PGX = PGX[WHERE(PGX GT -1)] & PGY = PGY[WHERE(PGY GT -1)]
          PSY = POLYLINE(PGX,PGY,LINESTYLE=STR.POLYLINE_LINESTYLE,/DEVICE, COLOR=STR.POLYLINE_COLOR, TRANSPARENCY=STR.POLYLINE_TRANSPARENCY, THICK=STR.POLYLINE_THICK, TARGET=IMG)          
        ENDFOR
      ENDIF ELSE BEGIN
        MAPS_SET, AMAP
        LL = MAP_DEG2IMAGE(MAPS_BLANK(AMAP),POLYLINE_LONS,POLYLINE_LATS,X=PGX,Y=PGY)
        PGX = PGX[WHERE(PGX GT -1)] & PGY = PGY[WHERE(PGY GT -1)]
        ZWIN
        PSY = POLYLINE(PGX,PGY,LINESTYLE=POLYLINE_LINESTYLE,/DEVICE, COLOR=POLYLINE_COLOR, TRANSPARENCY=POLYLINE_TRANSPARENCY, THICK=POLYLINE_THICK, TARGET=IMG)
      ENDELSE
    ENDIF
