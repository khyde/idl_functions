; $ID:	SAVE_MAKE_PRODS.PRO,	2023-09-21-13,	USER-KJWH	$
...
; ************************************************************************************************************************************************************
  ...
  DSET = VALIDS('SENSORS',DATASET)
  COVERAGE = VALIDS('COVERAGE',DATASET)

; ===> Defaults  
  IF ~N_ELEMENTS(MPS)       THEN MPS     = 'L3B2'
  IF ~N_ELEMENTS(PROD)      THEN PROD    = []
  IF ~N_ELEMENTS(SST_GR)    THEN SST_GR  = 'AVINTERP_MUR'
  IF ~N_ELEMENTS(PPD_ALG)   THEN PPD_ALG = 'VGPM2'
  IF ~N_ELEMENTS(MASK)      THEN MASK    = []
  IF ~N_ELEMENTS(DATERANGE) THEN DATERANGE = ['19780101',DATE_NOW(/DATE_ONLY)]
  
; ===> Loop through DATASETS
  FOR DTH=0, N_ELEMENTS(DATASET)-1 DO BEGIN
    DSET = DATASET[DTH]
    DIR = !S.(WHERE(TAG_NAMES(!S) EQ DSET,/NULL))
...
; ===> Set up the output file directories
        DIR_SAVE = DIR + AMAP  + SL + 'SAVE' + SL + APROD + SL
        IF APROD EQ 'PPD_SIZE' THEN DIR_SAVE = DIR + AMAP  + SL + 'STATS' + SL + APROD + '-' + PPD_ALG + SL
        DIR_TEST, DIR_SAVE
        DIR_STATS = DIR + AMAP + SL + 'STATS' + SL
...
; ===> Based on the input PROD name, determine the file search product (FPROD) and the name(s) of the prods inside the file (SPRODS)  
        ...
        CASE APROD OF
          ...
          'PHYTO_SIZE-TURNER': BEGIN & VERSION='V1' & FPROD='CHL' & SPRODS=['DATA','SST'] & END
          'PSC-TURNER': BEGIN & VERSION='V1' & FPROD='CHL' & SPRODS=['DATA','SST'] & END
          'POC-STRAMSKI':    BEGIN & VERSION='VER2' & FPROD='RRS' & SPRODS=['RRS_490','RRS_555'] & END
          ELSE: MESSAGE, 'ERROR: Unrecognized input PROD'
        ENDCASE

; ===> DATASET specific wavelengths
        IF DSET EQ 'MODISA' THEN SPRODS = REPLACE(SPRODS,['RRS_490','RRS_555','RRS_670'],['RRS_488','RRS_547','RRS_667']) ; Replace the SeaWiFS bands with the equivalent MODIS bands
        IF DSET EQ 'VIIRS'  THEN SPRODS = REPLACE(SPRODS,['RRS_490','RRS_555','RRS_670'],['RRS_486','RRS_551','RRS_671'])
        IF DSET EQ 'JPSS1'  THEN SPRODS = REPLACE(SPRODS,['RRS_490','RRS_555','RRS_670'],['RRS_489','RRS_556','RRS_667'])
        IF DSET EQ 'OCCCI'  THEN BEGIN
          FPROD  = REPLACE(FPROD,'CHL','CHLOR_A-CCI') 
          SPRODS = REPLACE(SPRODS,['RRS_555','RRS_670'],['RRS_560','RRS_665'])
          FILE_TYPE='SAVE' 
        ENDIF
        IF DSET EQ 'GLOBCOLOUR'  THEN BEGIN
...
; ===> Find the other product files based on the output product    
        SSTFILES=[]
        CASE APROD OF
          'PHYTO_SIZE-HIRATA_NES': FILES = GET_FILES(DSET,PRODS=FPROD,FILE_TYPE=FILE_TYPE, DATERANGE=DATERANGE, COUNT=COUNT)
          'PHYTO_SIZE-HIRATA':     FILES = GET_FILES(DSET,PRODS=FPROD,FILE_TYPE=FILE_TYPE, DATERANGE=DATERANGE, COUNT=COUNT)
          
          'PHYTO_SIZE-BREWIN_NES': BEGIN
            DIR_TEST,REPLACE(DIR_SAVE,'BREWIN_NES','BREWINSST_NES')
            FILES = GET_FILES(DSET, PRODS=FPROD, FILE_TYPE=FILE_TYPE, DATERANGE=DATERANGE, COUNT=COUNT)
            CASE SST_GR OF
          ...
          'PHYTO_SIZE-BREWINSST_NES': BEGIN
            ...
            CASE SST_GR OF
              'AVINTERP_MUR': BEGIN ; USE INTERPOLATED AVHRR UNTIL MUR DATA IS AVAILABLE IN 2002
                VFILES = GET_FILES('AVHRR',MAPS='L3B4',FILE_TYPE='INTERP',PRODS='SST',DATERANGE=DATERANGE,COUNT=VNUM) & VFP = PARSE_IT(VFILES)
                MFILES = GET_FILES('MUR',MAPS=AMAP,FILE_TYPE='SAVE',PRODS='SST',DATERANGE=DATERANGE,COUNT=MNUM) & MFP = PARSE_IT(MFILES)
                IF DATE_2JD(DATERANGE[0]) LT DATE_2JD('20020601') THEN SSTFILES = [DATE_SELECT(VFILES,[DATERANGE[0],'20020531']),MFILES] ELSE SSTFILES = MFILES
              END
              ELSE: MESSAGE, 'Unrecognized SST input'
            ENDCASE
          ...
          'PSC-TURNER': BEGIN
            IF DSET EQ 'OCCCI' THEN SPRODS = 'DATA' ELSE SPRODS = 'CHLOR_A'
            FILES = GET_FILES(DSET, PRODS=FPROD, MAPS=AMAP, FILE_TYPE=FILE_TYPE, DATERANGE=DATERANGE, COUNT=COUNT)
            CASE SST_GR OF
              'AVINTERP_MUR': BEGIN ; USE INTERPOLATED AVHRR UNTIL MUR DATA IS AVAILABLE IN 2002
                VFILES = GET_FILES('AVHRR',MAPS='L3B4',FILE_TYPE='INTERP',PRODS='SST',DATERANGE=DATERANGE,COUNT=VNUM) & VFP = PARSE_IT(VFILES)
                MFILES = GET_FILES('MUR',MAPS=AMAP,FILE_TYPE='SAVE',PRODS='SST',DATERANGE=DATERANGE,COUNT=MNUM) & MFP = PARSE_IT(MFILES)
          ...
          'PIGMENTS-PAN': BEGIN
            ...
            CASE SST_GR OF
              'AVINTERP_MUR': BEGIN ; USE INTERPOLATED AVHRR UNTIL MUR DATA IS AVAILABLE IN 2002
                VFILES = GET_FILES('AVHRR',MAPS='L3B4',FILE_TYPE='INTERP',PROD='SST',DATERANGE=DATERANGE,COUNT=VNUM) & VFP = PARSE_IT(VFILES)
                MFILES = GET_FILES('MUR',MAPS=AMAP,FILE_TYPE='SAVE',PROD='SST',DATERANGE=DATERANGE,COUNT=MNUM) & MFP = PARSE_IT(MFILES)
                IF DATE_2JD(DATERANGE[0]) LT DATE_2JD('20020601') THEN SSTFILES = [DATE_SELECT(VFILES,[DATERANGE[0],'20020531']),MFILES] ELSE SSTFILES = MFILES
              END
              ELSE: MESSAGE, 'Unrecognized SST input'
            ENDCASE
          ...
          'CHLOR_A-WERDELL': BEGIN
            IF DSET EQ 'OCCCI' THEN BEGIN
              SPRODS = 'DATA'
              FILES = FLS(!S.OC + DSET + SL + AMAP +SL + 'SAVE' + SL + 'CHLOR_A-OCI' + SL + '*' + 'CHLOR_A-OCI' + '*.SAV',DATERANGE=DATERANGE, COUNT=COUNT)
            ENDIF ELSE FILES = FLS(!S.OC + DSET + SL + AMAP +SL + 'NC'   + SL + FPROD         + SL + '*' + FPROD         + '*.nc', DATERANGE=DATERANGE, COUNT=COUNT)
          END
          
          'PPD_SIZE': BEGIN ; Search for the PHYTO and PPD stats files
            FILES    = FLS(!S.OC  + DSET + SL + AMAP + SL + 'STATS' + SL + FPROD            + SL + PERIOD + '_*' + FPROD   + '*.SAV', DATERANGE=DATERANGE,COUNT=COUNT)
            PPDFILES = FLS(!S.PP + DSET + SL + AMAP + SL + 'STATS' + SL + 'PPD-' + PPD_ALG + SL + 'M_*PPD-'     + PPD_ALG + '*.SAV', DATERANGE=DATERANGE, COUNT=PNUM)
...
; ===> Loop through files      
        ...
        FOR F=0, N_ELEMENTS(FILES)-1 DO BEGIN
          ...
          IF APROD EQ 'PSC-TURNER' AND SST_FILE EQ [] THEN CONTINUE ; >>>>>>>>>>>>>> Must have SST file to run TURNER PSIZE algorithm
          IF APROD EQ 'PHYTO-PAN' OR APROD EQ 'PPD_SIZE' THEN GOTO, SKIP_READ_NC ; >>>>>>>>>>>>>>>
          
          
          IF DSET EQ 'OCCCI' OR DSET EQ 'GLOBCOLOUR' THEN BEGIN
          ...
          'PSC-TURNER': BEGIN  ; Only save the BREWINSST_NES data and not the BREWIN_NES
            ...
            ENDIF ELSE BEGIN 
              IF STRUCT_HAS(STR,'BINS') THEN SST = MAPS_L3B_2ARR(SST,MP=AMAP,BINS=SSTBINS) ;IF N_ELEMENTS(STR.BINS) NE PY THEN BEGIN   ; If the structure has the BINS tag then recreate the full BIN
;              BLK = FLTARR(PY) & BLK[*] = MISSINGS(0.0)
;              SBLK = BLK
;              SBLK[SSTBINS-1] = SST
;              SST = FLTARR(1,PY)
;              SST[0,*] = SBLK & GONE, SBLK
;            ENDIF
          ENDELSE
          SZS = SIZEXYZ(SST,PX=SPX,PY=SPY)
          ...
          OK = WHERE(PHYTO.PSC_MICRO NE MISSINGS(0.0),COUNTM)
          IF IS_L3B(AMAP) AND COUNTM GT 0 THEN BEGIN
            PBINS = BINS[OK]
            NSTR = [] 
            TAGS = TAG_NAMES(PHYTO)
            FOR PHY=0, N_TAGS(PHYTO)-1 DO BEGIN
              TDAT = PHYTO.(PHY)
              NSTR = CREATE_STRUCT(NSTR,TAGS[PHY],TDAT[OK])
            ENDFOR
            PHYTO = NSTR & GONE, NSTR
          ENDIF ELSE PBINS = BINS
          
; TODO Look into using a HASH instead of a structure
; TODO Subset by BINS so missing data are not saved          
          STRUCT_WRITE, PHYTO, BINS=PBINS, FILE=SAVEFILE, DATA_UNITS=UNITS('CHLOR_A',/SI), INFILE=FILES[F], ALG='TURNER', PROD='PHYTO_SIZE', SATELLITE=SI.SATELLITE, SENSOR=SI.SENSOR, COVERAGE=SI.COVERAGE, LOGLUN=LOG_LUN
          GONE, PHYTO & GONE, PSIZE & GONE, PHYTOSST
          IF KEY(COMPOSITE) THEN PHYTO_COMPOSITE, SAVEFILE
        END ; PHYTO-PAN
