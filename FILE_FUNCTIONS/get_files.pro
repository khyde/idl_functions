...
; ****************************************************************************************************
  ...
  FOR D=0, N_ELEMENTS(DATASETS)-1 DO BEGIN
    ...
    CASE CLIMATOLOGY OF
      ...
      'DEFAULT': BEGIN & CLIMDTR = GET_DATERANGE(['1991','2020']) & IF HAS(PERIOD,'MONTH3') THEN CLIMDTR = GET_DATERANGE(['1991','2021']) & END
      'ALL': CLIMDTR = []
    ...
    FOR L=0, N_ELEMENTS(LEVELS)-1 DO BEGIN
      ...
      CASE DAT OF
        'OCCCI': BEGIN
          ...
          ENDCASE
          ;IF ~N_ELEMENTS(MAPS) THEN NCMAP = 'SOURCE' ELSE NCMAP = MAPS
          NCMAP = 'SOURCE'
        END
        ELSE: BEGIN & IF ~N_ELEMENTS(MAPS) THEN NCMAP = 'SOURCE' ELSE NCMAP = MAPS & DIR = !S.(WHERE(STAGS EQ DAT,/NULL))  & END
      ENDCASE

      ; ===> Establish the default NC dirs
      CASE DAT OF 
        'OCCCI': BEGIN & IF HAS(MAPS,'1KM') THEN NCSRC = 'BINNED_1KM_DAILY' ELSE NCSRC = 'BINNED_4KM_DAILY' & END
...
DR = []        
      ...
      FOR M=0, N_ELEMENTS(DMAP)-1 DO BEGIN
        ...
        FOR P=0, N_ELEMENTS(PRODS)-1 DO BEGIN
          ...
          IF DAT EQ 'GLORYS' THEN DIR_NC = REPLACE(NDIR_DAT,'V12','glorys_bottomT') + 'cmems_mod_glo_phy_my_0.083deg_P1D-m/' + APROD + SL
