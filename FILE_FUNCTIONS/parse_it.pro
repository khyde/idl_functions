...
; ===> If the name contains a "satdate" (an indicator it is a downloaded file and not a processed file) then run SENSOR_INFO for specific files
  OK = WHERE(IS_SATDATE(REPLACE(NAME,'.','-')) EQ 1 OR IS_NASADATE(REPLACE(NAME,'.','-')) EQ 1, COUNT) ; ===> IF L1A, L2, L3B, MUR OR AVHRR FILES, THEN USE SENSOR_INFO TO GET THE INFORMATION
  IF COUNT GE 1 THEN BEGIN
