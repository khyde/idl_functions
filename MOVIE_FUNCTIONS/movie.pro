; $ID:	MOVIE.PRO,	2023-09-21-13,	USER-KJWH	$
; ;########################################################################################3
PRO MOVIE, FILES, MOVIE_FILE=MOVIE_FILE, DIR_OUT=DIR_OUT, N_REPEAT=N_REPEAT, FPS=FPS, FORMAT=FORMAT, WIDTH=WIDTH, HEIGHT=HEIGHT, $
  ...
  IF KEYWORD_SET(ADD_TITLE) OR N_ELEMENTS(TITLE_FILE) EQ 1 THEN BEGIN
    IF NONE(TITLE_FILE) THEN BEGIN
      ...
      IF NONE(TITLE_TXT) THEN BEGIN
        IF DATE_TXT NE '' THEN TITLE_TXT = [DATE_TXT, 'Created by ' + METADATA.AUTHOR, REPLACE(METADATA.AFFILIATION,' | U.S. Department of Commerce','')] $
                          ELSE TITLE_TXT = ['Created by ' + METADATA.AUTHOR, REPLACE(METADATA.AFFILIATION,' | U.S. Department of Commerce','')]
