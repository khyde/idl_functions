  ...
  IF KEYWORD_SET(ADD_LONLAT) THEN BEGIN
    ...
    CASE MP OF
      ...
      'MAB_GS': BEGIN
        CASE RESZ OF
          0.85: BEGIN
            LAT = FLOAT(WHERE_NEAREST(MLL.LATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[27,-2,-32] & LATX = REPLICATE(PIXBL[1]+12,N_ELEMENTS(LAT))
            LON = FLOAT(WHERE_NEAREST(MLL.LONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[41,5,-30] & LONY = REPLICATE(PIXBL[0]-12,N_ELEMENTS(LON))
            LATPOS = 'L' & LONPOS = 'B'
          END
          ELSE: BEGIN
      ...
      'CHES_BAY': BEGIN
        LLFONT_SIZE = 10
        LAT = FLOAT(WHERE_NEAREST(MLL.LATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[32,0,-32] & LATX = REPLICATE(PIXBL[1]-30,N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLL.LONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[16,-10]   & LONY = REPLICATE(PIXBL[0]+30,N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'
      END  
      ELSE: BEGIN
        LAT = FLOAT(WHERE_NEAREST(MLL.LATS[0,*],FLOAT(LATS),NEAR=0.05))*XX & LATX = REPLICATE(0,N_ELEMENTS(LAT))
