; $ID:	PROJECT_MAKE_IMAGE.PRO,	2024-01-30-21,	USER-KJWH	$
  ...
  IMGOBJ=IMGOBJ, DATAPROD=DATAPROD, $
  ADD_BATHY=ADD_BATHY, BATHY_DEPTH=BATHY_DEPTH, $; BATHY_COLOR=BATHY_COLOR, BATHY_THICK=BATHY_THICK, $
  ADD_POINTS=ADD_POINTS, PLONS=PLONS, PLATS=PLATS, PSYM=PSYM, PCOLOR=PCOLOR, PSIZE=PSIZE, PFILL_COLOR=PFILL_COLOR, PFILL=PFILL, PTHICK=PTHICK,$
...
; ****************************************************************************************************
  ...
  IF N_ELEMENTS(DATAPROD) EQ 0 THEN PROD = FP.PROD ELSE PROD = DATAPROD
  ...
  IF KEYWORD_SET(ADD_LONLAT) THEN BEGIN
    IF ~N_ELEMENTS(LONS) THEN BEGIN
      CASE MP OF
        'MAB': LONS = [-76,-73,-70]
        'MAB_GS': LONS = [-75,-71,-67]
        'NJ_COAST': LONS = [-76,-75,-74,-73]
        'CHES_BAY': LONS = [-77,-76,-75]
    ...
    IF ~N_ELEMENTS(LATS) THEN BEGIN
      CASE MP OF
        'MAB': LATS = [36,38,40]
        'MAB_GS': LATS = [36,39,42]
        'NJ_COAST': LATS = [39,40]
        'CHES_BAY': LATS = [37,38,39]
        ELSE: LATS = [ 36, 40, 44]
      ENDCASE
    ENDIF
    ...
    MLL = MAPS_2LONLAT(MP)
    IF HAS(MLL,'LATS') THEN MLATS = MLL.LATS ELSE MLATS = MLL.LAT
    IF HAS(MLL,'LONS') THEN MLONS = MLL.LONS ELSE MLONS = MLL.LON
  ENDIF
  ...
  IF IDLTYPE(IMGOBJ) NE 'OBJREF' THEN BEGIN 
    IF KEYWORD_SET(STACKED) THEN BEGIN
      STACKED_2PNGS, FILE, OUTIMG=IMG, MAP_OUT=MP, PAL=USEPAL, SPROD=PSCL, ADD_BATHY=BATHY, OUTLINE=OUTLINE, OUT_COLOR=OUT_COLOR, OUT_THICK=OUT_THICK,$
        ...
        ;ADD_POINTS=ADD_POINTS,POINT_LONS=PLONS, POINT_LATS=PLATS, POINT_SYMBOL=PSYM, POINT_SIZE=PSIZE, POINT_FILL_COLOR=PFILL_COLOR, POINT_COLOR=PCOLOR, POINT_SYM_FILL=PFILL, POINT_THICK=PTHICK,$
  ...
  IF KEYWORD_SET(ADD_POINTS) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      ZWIN
      PSY = SYMBOL(PX*XX,PY*XX,SYMBOL=PSYM,/DEVICE, SYM_FILLED=PFILL, SYM_COLOR=PCOLOR, SYM_SIZE=PSIZE, TARGET=IMG)
      ;IF KEYWORD_SET(RESIZE) THEN PSY.SCALE,RESZ, RESZ
    ENDELSE
  ENDIF
  ...
  IF IMG EQ [] THEN GOTO, DONE
  IF KEYWORD_SET(RESZ) AND RESZ NE 1 THEN BEGIN
    CPIMG = IMG.COPYWINDOW()
    IMG.SCALE,RESZ,RESZ
    IPOS = IMG.POSITION
    IMG.CLOSE
    IMG = IMAGE(CPIMG, DIMENSIONS=MR_DIMS, POSITION=IPOS, BUFFER=BUFFER)
  ENDIF
  POS = IMG.POSITION
  ...
  CASE MP OF
    ...
    'MAB_GS': BEGIN 
      ...
      DB_POS=[0,0.95,1,1] 
      CASE RESZ OF
        0.85: BEGIN & CB_POS = [0.095, 0.85, 0.475, 0.875] & TITLE_POS=[0.5,0.96] & TITLE_SIZE=11 & END
  ...
  IF KEYWORD_SET(ADD_LONLAT) THEN BEGIN
    ...
    CASE MP OF
      'NES': BEGIN
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[34,0,-34] & LATX = REPLICATE(PIXBL[1],N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[39,13,-12,-38] & LONY = REPLICATE(PIXBL[0],N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'
      END
      'GOM': BEGIN
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[26,-1,-27] & LATX = REPLICATE(PIXBL[1],N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[37,1,-30] & LONY = REPLICATE(PIXBL[0],N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'
      END
      'MAB': BEGIN
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[35,2,-33] & LATX = REPLICATE(PIXBL[1]-10,N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[37,-4,-45] & LONY = REPLICATE(PIXBL[0]+10,N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'
      END  
      'MAB_GS': BEGIN
        CASE RESZ OF
          0.85: BEGIN
            LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[27,-2,-32] & LATX = REPLICATE(PIXBL[1]+12,N_ELEMENTS(LAT))
            LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[41,5,-30] & LONY = REPLICATE(PIXBL[0]-12,N_ELEMENTS(LON))
            LATPOS = 'L' & LONPOS = 'B'
          END
          ELSE: BEGIN
            LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX & LATX = REPLICATE(0,N_ELEMENTS(LAT))
            LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY & LONY = REPLICATE(0,N_ELEMENTS(LON))
            LATPOS = 'R' & LONPOS = 'T'
          END    
        ENDCASE
      END
      'NJ_COAST': BEGIN
        LLFONT_SIZE = 10
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[24,-8]     & LATX = REPLICATE(PIXBL[1]+0,N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[22.5,-4,-30] & LONY = REPLICATE(PIXBL[0]+0,N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'    
      END  
      'CHES_BAY': BEGIN
        LLFONT_SIZE = 10
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX+[32,0,-32] & LATX = REPLICATE(PIXBL[1]-30,N_ELEMENTS(LAT))
        LON = FLOAT(WHERE_NEAREST(MLONS[*,0],FLOAT(LONS),NEAR=0.05))*YY+[16,-10]   & LONY = REPLICATE(PIXBL[0]+30,N_ELEMENTS(LON))
        LATPOS = 'L' & LONPOS = 'B'
      END  
      ELSE: BEGIN
        LAT = FLOAT(WHERE_NEAREST(MLATS[0,*],FLOAT(LATS),NEAR=0.05))*XX & LATX = REPLICATE(0,N_ELEMENTS(LAT))
