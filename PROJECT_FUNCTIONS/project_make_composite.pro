; $ID:	PROJECT_MAKE_COMPOSITE.PRO,	2023-09-21-13,	USER-KJWH	$
  PRO PROJECT_MAKE_COMPOSITE, VERSTR, PRODS=PRODS, TYPES=TYPES, MAPP=MAPP, $
           YEARS=YEARS, MONTHS=MONTHS, WEEKS=WEEKS, DAYS=DAYS, YEAR_COMBO=YEAR_COMBO, COMPOSITE_MONTH=COMPOSITE_MONTH,$ 
           DATERANGE=DATERANGE, CLIMATOLOGY=CLIMATOLOGY,BOTTOM_COLORBAR=BOTTOM_COLORBAR, COLORBAR_TITLE=COLORBAR_TITLE,$
...
; ****************************************************************************************************
  ...
  IF KEYWORD_SET(COMPOSITE_MONTH) THEN BEGIN
    MONTHS = 0 & WEEKS = 0 & DAYS = 0 & YEAR = 1
    IF ~N_ELEMENTS(NCOLS) THEN NCOLS = 6 
    IF ~N_ELEMENTS(NROWS) THEN NROWS = 2
  ENDIF
  ...
  FOR M=0, N_ELEMENTS(MPS)-1 DO BEGIN
    ...
    IF KEYWORD_SET(COMPOSITE_MONTH) AND NFILES GT 1 THEN MESSAGE, 'ERROR: At this time COMPOSITE_MONTH can only handle one product and 1 type'
    ...
    CASE 1 OF
      ...
      KEYWORD_SET(MONTHS): BEGIN
        ...
        IF KEYWORD_SET(YEAR_COMBO) THEN PERS = [PERS,'MM_'+YRS[0] +MTHS + '_' + YRS[-1] + MTHS] ELSE FOR Y=0, N_ELEMENTS(YRS)-1 DO PERS = [PERS,'M_' + YRS[Y] + MTHS]
        PERDATES = MTHS
      END
      KEYWORD_SET(YEAR): BEGIN
        PERCODE = 'Y'
        FOR Y=0, N_ELEMENTS(YRS)-1 DO PERS = [PERS,'Y_'+YRS[Y]]
        PERDATES = YRS
      END
    ENDCASE
    ...
    FOR K=0, N_ELEMENTS(PERS)-1 DO BEGIN
      ...
      CASE PER.PERIOD_CODE OF
        'Y': BEGIN
          TXT = PER.YEAR_START
          ITXT = MONTH_NAMES()
          PYRS = MONTH_RANGE(/STRING)
          PLOT_YEAR = PER.YEAR_START
          YR_MONTHS = PLOT_YEAR + PYRS
        END  
        'M': TXT = MONTH_NAMES(PER.MONTH_START) + ' ' + STRMID(PER.DATE_START,0,4)
        'W': TXT = 'Week ' + STRMID(PER.PERIOD,6,2) + ': ' + STRMID(PER.DATE_START,0,8) + ' - ' + STRMID(PER.DATE_END,0,8)
        'WW': BEGIN 
          TXT = 'Week '  + STRMID(PER.PERIOD,7,2) + ': ' + MONTH_NAMES(STRMID(PER.DATE_START,4,2)) + ' ' + STRMID(PER.DATE_START,6,2) + ' - ' + $
                                                           MONTH_NAMES(STRMID(PER.DATE_END,4,2)) + ' ' + STRMID(PER.DATE_END,6,2) + $
                                                           '!C ' + STRMID(PER.DATE_START,0,4) + ' - ' + STRMID(PER.DATE_END,0,4)
          ITXT = 'Week ' + STRMID(PER.PERIOD,7,2) + ': ' + MONTH_NAMES(STRMID(PER.DATE_START,4,2)) + ' ' + STRMID(PER.DATE_START,6,2) + ' - ' + $
                                                           MONTH_NAMES(STRMID(PER.DATE_END,4,2)) + ' ' + STRMID(PER.DATE_END,6,2)
        END
        'MM': BEGIN
          TXT = MONTH_NAMES(STRMID(PER.DATE_START,4,2)); + '!C ' + STRMID(PER.DATE_START,0,4) + ' - ' + STRMID(PER.DATE_END,0,4) 
          ITXT = MONTH_NAMES(STRMID(PER.DATE_START,4,2)) 
        END
      ...
      IF KEYWORD_SET(COMPOSITE_MONTH) THEN PLOTPRDS = 12
      IF KEYWORD_SET(BOTTOM_COLORBAR) THEN ADD_COLORBAR=0

    
      ; ===> SET UP PLOT SPACING
      IF ~N_ELEMENTS(BUFFER) THEN BUFFER = 1 ; Do plotting in background
      IF ~N_ELEMENTS(SPACE)  THEN SPACE  = 0.5
      IF ~N_ELEMENTS(LEFT)   THEN LEFT   = 1
      IF ~N_ELEMENTS(RIGHT)  THEN RIGHT  = 1
      IF ~N_ELEMENTS(TOP)    THEN TOP    = 10
      IF ~N_ELEMENTS(BOTTOM) THEN IF KEYWORD_SET(BOTTOM_COLORBAR) THEN BOTTOM = 20 ELSE BOTTOM = SPACE * 3
      ...
      FOR N=0, N_ELEMENTS(PRDS)-1 DO BEGIN
        ...
        FOR T=0, N_ELEMENTS(TPS)-1 DO BEGIN
          ATYPE = TPS[T]
          IF KEYWORD_SET(COMPOSITE_MONTH) THEN FILE = GET_FILES(DSET,PRODS=PSTR.PROD,PERIOD='M',FILE_TYPE=ATYPE, DATERANGE=DR, CLIMATOLOGY=CLIM) $
                                          ELSE FILE = GET_FILES(DSET,PRODS=PSTR.PROD,PERIOD=PER.PERIOD_CODE,FILE_TYPE=ATYPE, DATERANGE=DR, CLIMATOLOGY=CLIM)
          IF STRUCT_HAS(PSTR,'TEMP_PROD') THEN TEMP_PROD=PSTR.TEMP_PROD ELSE TEMP_PROD = PSTR.PROD
          IF FILE EQ [] THEN FILE = GET_FILES(PSTR.TEMP_DATASET,PRODS=TEMP_PROD,PERIOD=PER.PERIOD_CODE,FILE_TYPE=ATYPE, DATERANGE=DR)
...
;          IF PR.IN_PROD NE PR.PROD THEN PROD_SCALE = PR.IN_PROD ELSE PROD_SCALE = PSTR.PROD_SCALE;PSTR.PROD_SCALE
          
          IF HAS(ATYPE,'ANOM') THEN PRODPAL = PSTR.ANOM_PAL ELSE PRODPAL = PSTR.PAL 
          IF HAS(ATYPE,'ANOM') THEN PROD_SCALE = PSTR.ANOM_SCALE ELSE PROD_SCALE = PSTR.PROD_SCALE
          IF HAS(ATYPE,'ANOM') THEN CBTITLE = PSTR.ANOM_TITLE ELSE CBTITLE = UNITS(PSTR.PROD)
          IF KEYWORD_SET(COLORBAR_TITLE) THEN CBTITLE = COLORBAR_TITLE         
          ...
          FOR Y=0, N_ELEMENTS(PYRS)-1 DO BEGIN
            ...
            IF KEYWORD_SET(COMPOSITE_MONTH) THEN YDR = GET_DATERANGE(PLOT_YEAR+AYEAR,PLOT_YEAR+AYEAR)
            ...
            ENDIF ELSE BEGIN
              ...
              IF C EQ 0 THEN BEGIN
                IF COUNTER GT 1 THEN NEWROW = NEWROW + 1
                R = NEWROW
              ENDIF ELSE R = NEWROW ; When C = 0, start a new row
            ...
            FILE = []
            IF KEYWORD_SET(COMPOSITE_MONTH) THEN FILE = GET_FILES(DSET,PRODS=PSTR.PROD,PERIOD='M',FILE_TYPE=ATYPE, DATERANGE=PLOT_YEAR) $
                                            ELSE FILE = GET_FILES(DSET,PRODS=PSTR.PROD,PERIOD=PER.PERIOD_CODE,FILE_TYPE=ATYPE, DATERANGE=AYEAR)
            

            IF N_ELEMENTS(FILE) GT 1 THEN MESSAGE,'ERROR: More that one file found for ' + PSTR.PROD + ' - ' + ATYPE
   
            IMG = []
            IF FILE NE [] THEN IMG = PROJECT_MAKE_IMAGE(VERSTR, FILE=FILE, DATERANGE=YDR,PERIOD=YPER, BUFFER=BUFFER, RESIZE=RESZ, MAPP=MP, PROD_SCALE=PROD_SCALE, CBTITLE=CBTITLE, PAL=PRODPAL, ADD_CB=ADD_CB, _EXTRA=EXTRA) 
            IF IDLTYPE(IMG) NE 'OBJREF' THEN BEGIN
              ...
              IF TSET NE [] THEN BEGIN
                IF KEYWORD_SET(COMPOSITE_MONTH) THEN FILE = GET_FILES(TSET,PRODS=TEMP_PROD,PERIOD='M',FILE_TYPE=ATYPE, DATERANGE=PLOT_YEAR) $
                                                ELSE FILE = GET_FILES(TSET,PRODS=TEMP_PROD,PERIOD=PER.PERIOD_CODE,FILE_TYPE=ATYPE, DATERANGE=AYEAR)
              ENDIF
              IF FILE EQ [] THEN CONTINUE
              IF N_ELEMENTS(FILE) GT 1 THEN MESSAGE,'ERROR: More that one file found for ' + PSTR.PROD + ' - ' + ATYPE
              IMG = PROJECT_MAKE_IMAGE(VERSTR, FILE=FILE, DATERANGE=YDR,PERIOD=YPER, BUFFER=BUFFER, RESIZE=RESZ, MAPP=MP, PROD_SCALE=PROD_SCALE, ADD_CB=ADD_CB,_EXTRA=EXTRA)
            ...
            IF KEYWORD_SET(COMPOSITE_MONTH) THEN IPNG = DIR_PNG + REPLACE(FP.NAME +'.PNG',[FP.PERIOD,FP.MAP],['M_'+PLOT_YEAR+AYEAR,MP]) $
                                            ELSE IPNG = DIR_PNG + REPLACE(FP.NAME +'.PNG',[FP.PERIOD,FP.MAP],[YPER,MP])
            IF STRUCT_HAS(FP,'MAP_SUBSET') THEN IPNG = REPLACE(IPNG,FP.MAP_SUBSET+'-','')
            IF STRUCT_HAS(FP,'PXY') THEN IPNG = REPLACE(IPNG,FP.PXY+'-','')
            
            IF IDLTYPE(IMG) EQ 'OBJREF' THEN BEGIN
              CPIM = IMG.COPYWINDOW()
              IF KEYWORD_SET(YEAR_COMBO) OR KEYWORD_SET(COMPOSITE_MONTH) THEN BEGIN
                IF KEYWORD_SET(COMPOSITE_MONTH) THEN TX = TEXT(0.5,0.95,ITXT[Y]+' '+PLOT_YEAR,ALIGNMENT=0.50,FONT_SIZE=FONT_SIZE) $
                                                ELSE TX = TEXT(0.5,0.95,ITXT+' '+AYEAR,ALIGNMENT=0.50,FONT_SIZE=FONT_SIZE)
                
              ENDIF ELSE TX = TEXT(0.5,0.95,TXT,ALIGNMENT=0.50,FONT_SIZE=FONT_SIZE)
            ...
            IF W EQ [] THEN BEGIN
              ...
              IF NCOL GE 6 THEN TFONT_SIZE = FONT_SIZE*0.5 ELSE TFONT_SIZE = FONT_SIZE
              TX = TEXT(WIDTH/2, HEIGHT-TOP, TXT, /DEVICE, FONT_STYLE='BOLD', FONT_SIZE=TFONT_SIZE, ALIGNMENT=0.5, VERTICAL_ALIGNMENT=0.5,TARGET=W)
              IF KEYWORD_SET(BOTTOM_COLORBAR) THEN BEGIN
                CBAR, PROD_SCALE, OBJ=W, FONT_SIZE=FONT_SIZE*.25, CB_TYPE=2, CB_POS=[0.25,0.03,0.75,0.06], CB_TITLE=CBTITLE, PAL=PRODPAL,RELATIVE=CB_RELATIVE, CB_OBJ=CB_IMG
              ENDIF
            ENDIF
            W.WINDOW.SETCURRENT
            IF IDLTYPE(IMG) EQ 'OBJREF' THEN BEGIN
              IMG = IMAGE(CPIM, /CURRENT, POSITION=IPOS, /DEVICE)
              IF NCOL GT 6 THEN TFONT_SIZE = FONT_SIZE*0.4 ELSE TFONT_SIZE = FONT_SIZE
              IF KEYWORD_SET(YEAR_COMBO) THEN IF R EQ 1 THEN TX = TEXT(IPOS[0]+(IPOS[2]-IPOS[0])/2,IPOS[1]-SPACE/2,AYEAR,/DEVICE,ALIGNMENT=0.5,VERTICAL_ALIGNMENT=1, FONT_SIZE=TFONT_SIZE)
              IF KEYWORD_SET(COMPOSITE_MONTH) THEN BEGIN
                IF KEYWORD_SET(BOTTOM_COLORBAR) THEN TX = TEXT(IPOS[0]+SPACE*18,IPOS[3]-TOP,ITXT[Y],/DEVICE,ALIGNMENT=0,VERTICAL_ALIGNMENT=1, FONT_SIZE=5) $
                                                ELSE TX = TEXT(IPOS[2]-SPACE*8,IPOS[1]+BOTTOM*2,ITXT[Y],/DEVICE,ALIGNMENT=1.0,VERTICAL_ALIGNMENT=1, FONT_SIZE=5)
                ENDIF
            ENDIF
          ENDFOR ; PRODS
        ENDFOR ; TYPES
