...
; ===> Verify files
  ...
  IF FP[0].MATH EQ 'STACKED_STATS' THEN STACKED = 1 ELSE STACKED = 0
    ...
    FOR M=0, N_ELEMENTS(MONTHS)-1 DO BEGIN
      ...
      IF KEYWORD_SET(STACKED) THEN BEGIN
        ...
        IF DAT EQ [] THEN BEGIN
          DAT = STACKED_READ(FILES[OK],DB=DB,BINS=BINS,INFO=INFO,PRODS=PRODS,KEYS=KEYS)
          SDATA =  GET_TAG(DAT,STATPROD)
          SPER = PERIOD_2STRUCT(DB.PERIOD) 
        ENDIF
        OK = WHERE(SPER.MONTH_START EQ AMONTH,COUNT)
        IF COUNT EQ 0 THEN CONTINUE
        PDATA = MAPS_REMAP(SDATA[*,*,OK],MAP_IN=INFO.MAP,MAP_OUT=MAP_OUT,BINS=BINS)
      ENDIF ELSE BEGIN
