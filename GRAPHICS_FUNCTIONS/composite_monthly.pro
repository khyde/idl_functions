; $ID:	COMPOSITE_MONTHLY.PRO,	2023-09-21-13,	USER-KJWH	$
...
PRO COMPOSITE_MONTHLY, FILES, FILEPROD, IMGPROD=IMGPROD, STATPROD=STATPROD, MAP_OUT=MAP_OUT, DIR_OUT=DIR_OUT, OUTFILE=OUTFILE, DATERANGE=DATERANGE, YEARS=YEARS, MONTHS=MONTHS,$
                        PAL=PAL, COLORBAR_TITLE=COLORBAR_TITLE, SECOND_COLORBAR_TITLE=SECOND_COLORBAR_TITLE, CB_TICKNAMES=CB_TICKNAMES, CB_TICKVALS=CB_TICKVALS, SUB_TICKNAMES=SUB_TICKNAMES, SUB_TICKVALS=SUB_TICKVALS, $
                        FONT_SIZE=FONT_SIZE, LAND_COLOR=LAND_COLOR, COAST_COLOR=COAST_COLOR, XDIM=XDIM, YDIM=YDIM, NROWS=NROWS, NCOLS=NCOLS, TOP=TOP, BOTTOM=BOTTOM, LEFT=LEFT, RIGHT=RIGHT, SPACE=SPACE,$
...
;   IMGPROD.............. The "product" for the images (to be passed to PRODS_2BYTE)
...
; ****************************************************************************************************
  ...
  IF ~N_ELEMENTS(FILES) THEN GOTO, DONE
...
; ===> Verify files
  ...
  IF FP[0].PERIOD_CODE NE 'M' AND FP[0].PERIOD_CODE NE 'MONTH' AND FP[0].PERIOD_CODE NE 'MM' THEN MESSAGE, 'ERROR: Input files must have the period code M or MONTH'
  IF SAME(FP.PROD) EQ 0 THEN MESSAGE, 'ERROR: All input files should have the same product'
  IF HAS(FP[0].L2SUB, 'STACKED') THEN STACKED = 1 ELSE STACKED = 0
...
; ===> Set up defaults  
  ...
  IF NONE(OUTFILE)        THEN OFILE = DIR_OUT + FP[0].NAME + '-' + 'COMPOSITE.PNG'
...
; ===> Get years and months from the files    
  ...
  IF NONE(MONTHS) THEN MONTHS = MONTH_RANGE(MIN(FP.MONTH_START),MAX(FP.MONTH_END),/STRING) 
  NAMES  = MONTH_NAMES(MONTH)
  IF FP[0].PERIOD_CODE EQ 'MONTH' THEN YEARS = ''
  IF N_ELEMENTS(YEARS) EQ 1 THEN BEGIN
    IF N_ELEMENTS(MONTHS) EQ 12 THEN OFILE = REPLACE(OFILE,SL+'M-',SL+'M_'+YEARS+'-') ELSE OFILE = REPLACE(OFILE,SL+'M-',SL+'M_'+YEARS+MONTHS[0]+'_'+YEARS+MONTHS[-1]+'-') 
  ENDIF
  IF N_ELEMENTS(YEARS) GT 1 THEN OFILE = REPLACE(OFILE,SL+'M-',SL+'M_'+MIN(YEARS)+'_'+MAX(YEARS)+'-')
  IF FP[0].PERIOD_CODE EQ 'MONTH' THEN OFILE = REPLACE(OFILE,SL+'M-',SL+'MONTH-')
  OFILE = REPLACE(OFILE,'--','-')
  IF FILE_MAKE(FILES,OFILE,OVERWRITE=OVERWRITE) EQ 0 THEN GOTO, DONE ; If the file exists goto the end of the program
...
; ===> Set up plot spacing  
  ...
  CBAR, IMGPROD, CB_TYPE=3, FONT_SIZE=CFONT_SIZE, CB_POS=CBPOS, CB_TITLE=COLORBAR_TITLE, PAL=PAL, OBJ=W
  
  IF SECOND_COLORBAR_TITLE NE [] THEN BEGIN
    ...
    CBAR, IMGPROD, IMG=OBJ, FONT_SIZE=CFONT_SIZE, CB_TYPE=3, CB_POS=CB_POS, CB_TITLE=SECOND_COLORBAR_TITLE, PAL=PAL, CB_TICKVALUES=SUB_TICKNAMES, CB_TICKNAMES=SUB_TICKVALS,RELATIVE=CB_RELATIVE, CB_OBJ=CB_IMG2
  ENDIF
  COUNTER = 0
      ...
      IF KEYWORD_SET(STACKED) THEN BEGIN
        ...
        IF FP.L2SUB EQ 'STACKED_ANOMS'  THEN IF ~N_ELEMENTS(ANOMPRODS) THEN BEGIN
          CASE STACKED_PRODS OF
            ...
            'BTEMP':  STACKED_PRODS = STACKED_PRODS + '_DIF'
            ELSE:  STACKED_PRODS = STACKED_PRODS + '_RATIO'
          ENDCASE
        ENDIF ELSE STACKED_PRODS = STACKED_PRODS + '_' + ANOMPRODS
        IF STACKED_PRODS EQ '' THEN MESSAGE, 'ERROR: The input PROD is not a VALID product name' 
        
        IF N_ELEMENTS(STACKED_PRODS) GT 1 THEN MESSAGE, 'ERROR: More than 1 PROD, update the STACKED_PRODS code accordingly'
        
        IF DAT EQ [] THEN BEGIN
          DAT = STACKED_READ(FILE,DB=DB)
          IF HAS(DAT,'BINS') THEN DBINS = DAT.BINS ELSE DBINS = []
          SDATA =  GET_TAG(DAT,STACKED_PRODS)
          SPER = PERIOD_2STRUCT(DB.PERIOD) 
        ENDIF
        OK = WHERE(SPER.MONTH_START EQ AMONTH,COUNT)
        IF COUNT EQ 0 THEN CONTINUE
        PDATA = MAPS_REMAP(SDATA[*,*,OK],MAP_IN=FP.MAP,MAP_OUT=MAP_OUT,BINS=DBINS)
      ENDIF ELSE BEGIN
        ...
        COUNTER = COUNTER + 1
        IF COUNT EQ 0 THEN CONTINUE
        PDATA = STRUCT_READ(FILES[OK],STRUCT=STR,MAP_OUT=MAP_OUT)   ; Read data file
      ENDELSE
      IM = PRODS_2BYTE(PDATA, PROD=IMGPROD)                       ; Extract data and convert to BYTE
  ...
  IF KEYWORD_SET(VERBOSE) THEN PRINT, 'Writing ' + OFILE
  W.SAVE, OFILE, RESOLUTION=RESOLUTION
  W.CLOSE
