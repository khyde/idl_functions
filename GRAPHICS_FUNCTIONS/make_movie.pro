; $ID:	MAKE_MOVIE.PRO,	2023-09-21-13,	USER-KJWH	$
; ;########################################################################################3
PRO MAKE_MOVIE, FILES, MOVIE_FILE=MOVIE_FILE, DIR_OUT=DIR_OUT, N_REPEAT=N_REPEAT, FRAME_SEC=FRAME_SEC, FORMAT=FORMAT, WIDTH=WIDTH, HEIGHT=HEIGHT, $
           ADD_TITLE=ADD_TITLE, TITLE_FILE=TITLE_FILE, N_TITLE=N_TITLE, TITLE_TXT=TITLE_TXT, AUTHOR=AUTHOR, AFFILIATION=AFFILIATION, $
...
;   AUG 18, 2021 - KJWH: Changed name from MOVIE to MAKE_MOVIE to avoid conflict with IDL's MOVIE program     
;                        Moved to GRAPHICS functions                                         
;   JUN 24, 2022 - KJWH: Added AUTHOR and AFFILIATION keywords
;                        *** Note there is a bug that changes the case of !S.AUTHOR and !S.AFFILIATION to !S.author and !S.affiliation
; -
; ****************************************************************************************************************************
  ROUTINE_NAME = 'MAKE_MOVIE'
  COMPILE_OPT IDL2
  
;===> DEFAULTS & CONSTANTS
  FORMATS = ['MP4','AVI', 'GIF',  'MOV', 'WEBM']
  UL='_'
  IF ~N_ELEMENTS(FRAME_SEC)   THEN FPS = 10 ELSE FPS = FRAME_SEC
  IF ~N_ELEMENTS(N_TITLE)     THEN N_TITLE = 10
  IF ~N_ELEMENTS(N_END)       THEN N_END = 10
  IF ~N_ELEMENTS(N_REPEAT)    THEN N_REPEAT = 1
  IF ~N_ELEMENTS(AUTHOR)      THEN AUTHOR = !S.AUTHOR 
  IF ~N_ELEMENTS(AFFILIATION) THEN AFFILIATION = !S.AFFILIATION
  IF !S.INITIALS EQ 'JEOR' THEN DEFAULT_FORMAT = 'AVI' ELSE DEFAULT_FORMAT = 'MP4'
  
  IF N_ELEMENTS(METADATA) EQ 0 THEN METADATA = CREATE_STRUCT('AUTH', 'Created by ' + AUTHOR, 'AFF', REPLACE(AFFILIATION,' | U.S. Department of Commerce',''))
  ...
  IF KEYWORD_SET(ADD_TITLE) OR N_ELEMENTS(TITLE_FILE) EQ 1 THEN BEGIN
    IF NONE(TITLE_FILE) THEN BEGIN
      ...
      IF NONE(TITLE_TXT) THEN BEGIN
        IF DATE_TXT NE '' THEN TITLE_TXT = [DATE_TXT, METADATA.AUTH, METADATA.AFF] $
                          ELSE TITLE_TXT = [METADATA.AUTH, METADATA.AFF]
...
; ===> Add metadata to movie file
  ...
  FOR MTH=0, N_TAGS(METADATA)-1 DO BEGIN
    CASE MTAGS[MTH] OF
      'AUTH': MTAG = 'Author'
      'AFF':  MTAG = 'Affiliation'
      ELSE:   MTAG = MTAGS[MTH]
    ENDCASE
    OVID.SETMETADATA,MTAG,METADATA.(MTH)
  ENDFOR
...
; ===> Add the images to the video
  ...
  FOR NTH = 0L, FRAMES-1 DO BEGIN
    ...
    IF NTH EQ 0 THEN SZ0 = SIZEXYZ(IMG, PX=PX0, PY=PY0, PZ=PZ1)
    SZ = SIZEXYZ(IMG, PX=PX, PY=PY, PZ=PZ)
    IF PX NE PX0 OR PY NE PY0 THEN BEGIN
      MESSAGE, 'ERROR: the deminsions of ' + FILES[NTH] + ' do not match the original image',/CONTINUE
      FILE_DELETE, FILES[NTH],/VERBOSE
      CONTINUE 
    ENDIF
