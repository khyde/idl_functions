; $ID:	IMG_TRUE_2EIGHT.PRO,	2020-06-03-17,	USER-KJWH	$
;+
;;#############################################################################################################
	PRO IMG_TRUE_2EIGHT,FILE,DIR_OUT=DIR_OUT,IMG=IMG
...
; CATEGORY:	IMG FAMILY;		 
;
; CALLING SEQUENCE: RESULT = IMG_TRUE_2EIGHT(FILE)
;
; INPUTS: FILE NAME  
...
;		
;; EXAMPLES:
;      IMG_TRUE_2EIGHT,!S.IMAGES + 'MASK_NEC_ESTUARY_SHELF.BMP'
;	NOTES:
...
;===> USE IMAGE
I = IMAGE(IM,IMAGE_DIMENSIONS = [PX,PY],DIMENSIONS = [PX,PY],/BUFFER)
I.SAVE,!S.IDL_TEMP +'JUNK.PNG',BIT_DEPTH = 1,RESOLUTION = 600,BORDER = 0
I.CLOSE
IMG = READ_PNG(!S.IDL_TEMP +'JUNK.PNG',R,G,B)

;===> RESIZE TO SAME DIMENSIONS AS INPUT IMG
IMG = CONGRID(IMG,PX,PY)
FN = FILE_PARSE(FILE)
OUTFILE = DIR_OUT + FN.NAME + '-8B.PNG'
WRITE_PNG,OUTFILE,IMG,R,G,B & PFILE,OUTFILE
