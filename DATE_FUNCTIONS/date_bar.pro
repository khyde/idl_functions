...
;     JUL 20, 2022 - KJWH: Added BACKGROUND_COLOR to the PLOT command                     
;########################################################################################
...
;	===> IF DATE IS ACTUALLY JULIAN (NON STRING) THEN ASSUME INPUT IS JD AND CONVERT IT TO DATE
  ...
  ;#################################### OBJECT GRAPHICS ###############################################################
    ...
    PL=PLOT([MIN(JDS)-LFADD,MAX(JDS)+RTADD],[0,1],/CURRENT,XRANGE=XRANGE,YMAJOR=0,XMAJOR=0,YMINOR=0,XMINOR=0,XSTYLE=1,YSTYLE=1,AXIS_STYLE=2,$
      POSITION=DB_POS,XTHICK=THICK_FRAME,YTHICK=THICK_FRAME,XCOLOR=FRAME_COLOR,YCOLOR=FRAME_COLOR,/NODATA,DEVICE=DEVICE,BACKGROUND_COLOR=DB_BKG)
    YPOS = 0.5
