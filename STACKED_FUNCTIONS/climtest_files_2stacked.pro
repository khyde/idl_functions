...
PRO CLIMTEST_FILES_2STACKED, FILES, PRODS=PRODS, STAT_TYPES=STAT_TYPES, D3_FILES=D3_FILES, DIR_OUT=DIR_OUT, MAP_OUT=MAP_OUT, L3BSUBMAP=L3BSUBMAP, FILE_LABEL=FILE_LABEL, $
  MAINPROD=MAINPROD, DATERANGE=DATERANGE, OUTFILE=OUTFILE, LOGLUN=LOGLUN, STACKED_TEMP=STACKED_TEMP, VERBOSE=VERBOSE, TESTING=TESTING, OVERWRITE=OVERWRITE
...
;
; KEYWORD PARAMETERS:
;   STACKED_TEMP.... If set, will create "temporary" stacked files of daily, weekly, or monthly files that will be used when calculating the climatological stats
...
;  Jan 04, 2024 - KJWH: Changed DOY keyword to STACKED_TEMP so that the program will work with weekly and monthly files in addition to daily
;-
; ****************************************************************************************************
  ...
  IF KEYWORD_SET(STACKED_TEMP) THEN BEGIN
    CASE PERIOD_CODE OF
      'DD': OUTPERIOD = 'DOY'                                                                                                   ; Change the output period to DOY
      'WW': OUTPERIOD = 'WEEK'                                                                                                  ; Change the output period to WEEK
      'MM': OUTPERIOD = 'MONTH'                                                                                                 ; Change the output period to MONTH
    ENDCASE
  ENDIF
  ...
  IF STRPOS(FP[0].L2SUB,'NC')      GE 0 THEN NC_INFILE = 1      ELSE NC_INFILE = 0                                              ; Determine if the input files are netcdf files
  ...
  FOR M=0, N_ELEMENTS(MAPS)-1 DO BEGIN
    ...
    IF FP[0].MAP_SUBSET NE '' THEN BEGIN                                                                                          ; Check to see if the input file name already has a SUBSET map
    ...
    IF N_ELEMENTS(DIR_OUT) NE 1 THEN DIROUT = REPLACE(FP[0].DIR,['/SAVE','/NC','/STATS','/ANOMS',FP[0].SUB],[REPLICATE('/STACKED_SAVE',4),PROD_LABEL])  $        ; Set up the output directory
                                                            ELSE DIROUT = DIR_OUT
      DIROUT = REPLACE(DIROUT,[SL+['SOURCE','SOURCE_MONTHLY','SOURCE_1KM']+SL,FP[0].MAP],[REPLICATE(SL+AMAP+SL,3),AMAP])                                                                                     ; Change the map in the output directory
      IF SI NE [] THEN DIROUT = REPLACE(DIROUT,SI.COVERAGE,AMAP)                                                                                     ; Change the map/coverage in the output directory
    IF KEYWORD_SET(STACKED_TEMP) THEN DIROUT = REPLACE(DIROUT,['STACKED_SAVE','STACKED_STATS'],['STACKED_TEMP','STACKED_TEMP'])                                             ; If DOY files, then save in the TEMP directory
    ...
    FOR N=0, N_ELEMENTS(PERIOD_SET)-1 DO BEGIN
      ...
      IF KEYWORD_SET(STACKED_TEMP) THEN BEGIN
        PSTR = PERIODS_READ(OUTPERIOD) & PERLEN = PSTR.FIRST_LENGTH + PSTR.SECOND_LENGTH + 1
        TPFILES = FILE_SEARCH(DIROUT + STRMID(PERIOD_NAME,0,PERLEN) + '*-' + _FILE_LABEL+'-STACKED.SAV',COUNT=COUNTTEMP)
        IF COUNTTEMP GT 0 THEN BEGIN
          IF COUNTTEMP GT 1 THEN MESSAGE, 'ERROR: Need to figure out how to work with more than one TEMP file'
          TPHASH = STACKED_READ(TPFILES,BINS=BINS, DB=TPDB,KEYS=TPKEYS)
          OKTP = WHERE_MATCH(TPDB.PERIOD,D3HASH['FILE_DB','PERIOD'], COUNT,VALID=VALID,INVALID=INVALID,COMPLEMENT=COMP)
          FOR V=0, N_ELEMENTS(D3_VARS)-1 DO BEGIN
            DVAR = D3_VARS[V]
            TPPOS = WHERE(TAG_NAMES(TPHASH) EQ DVAR,COUNT)
            IF COUNT NE 1 THEN MESSAGE, 'ERROR: ' + DVAR + ' not found in the existing DOYHASH file'
            D3HASH[DVAR,*,*,OKTP] = TPHASH.(TPPOS)[*,*,VALID]
  
            ; ===> Add the file information to the D3 database in the D3HASH
            D3HASH['FILE_DB','MTIME',OKTP]          = TPDB.MTIME[VALID]                                                           ; Add the file MTIME to the D3 database
            IF HAS_TAG(DB,'FULLNAME') THEN D3HASH['FILE_DB','FULLNAME',OKTP] = TPDB.FULLNAME[VALID]                                                        ; Add the full file name to the D3 database
            IF HAS_TAG(DB,'FULLNAME') THEN D3HASH['FILE_DB','NAME',OKTP]     = TPDB.NAME[VALID]  
            IF HAS_TAG(DB,'STATFILE') THEN D3HASH['FILE_DB','STATFILE',OKTP] = TPDB.STATFILE[VALID]                                                        ; Add the full file name to the D3 database
            IF HAS_TAG(DB,'STATNAME') THEN D3HASH['FILE_DB','STATNAME',OKTP] = TPDB.STATNAME[VALID]                                                          ; Add the file name to the D3 database
            D3HASH['FILE_DB','DATE_RANGE',OKTP]     = TPDB.DATE_RANGE[VALID] 
            D3HASH['FILE_DB','INPUT_FILES',OKTP]    = TPDB.INPUT_FILES[VALID]                                                     ; Add the "input" files to the D3 database (if present)
            D3HASH['FILE_DB','ORIGINAL_FILES',OKTP] = TPDB.ORIGINAL_FILES[VALID]                                                  ; Add the "original" files to the D3 database (if present)
   ...
   ;       OK = WHERE(DBDATE GE MIN(FP[NTH].DATE_START) AND DBDATE LE MAX(FP[NTH].DATE_END),COUNT)                                 ; Find the periods within the input period range
          ...
          IF KEYWORD_SET(STACKED_TEMP) AND COUNT EQ 0 THEN CONTINUE
   ...
   ;     ENDFOR
      ...
      FOR NTH=0, N_ELEMENTS(PERIODS)-1 DO BEGIN                                                                                   ; Loop through the periods
        ...
        SEQ = WHERE(D3HASH['FILE_DB','PERIOD'] EQ APERIOD,COUNT)                                                                  ; Find the period in the DB database
          ...
          ENDIF ELSE BEGIN                                                                                                        ; If file extension is .nc, get the ncprod data and the bins from the netcdf file
            ...
            FOR R=0, N_ELEMENTS(NPRODS)-1 DO BEGIN
              ...
              IF KEYWORD_SET([OCCCI_1KM,STACKED_NC]) THEN BEGIN
                IF KEYWORD_SET(OCCCI_1KM) THEN IMG = MAPS_OCCCI_LONLAT_2BIN(IMG, AMAP, MAP_IN='1KM', LATS=LATS, LONS=LONS) 
        ...
        IF HAS(DB,'STATFILE') THEN D3HASH['FILE_DB','STATFILE'] = FILE ELSE D3HASH['FILE_DB','FULLNAME',SEQ] = FILE                                                                                       ; Add the full file name to the D3 database
        IF HAS(DB,'STATNAME') THEN D3HASH['FILE_DB','STATNAME'] = FA.NAME ELSE D3HASH['FILE_DB','NAME',SEQ] = FA.NAME                                                                                        ; Add the file name to the D3 database
        ...
        FOR S=0, N_ELEMENTS(D3_VARS)-1 DO BEGIN
          ...
          IF DAT EQ [] THEN BEGIN                                                                                                     ; Check to make sure DAT is not !NULL
            IF VALIDS('PRODS',STR.PROD) EQ DATA_TAG THEN BEGIN
              DAT = GET_TAG(STR,'IMAGE')                                                                                              ; Use the "IMAGE" data tag to extract the data from the structure
              IF DAT EQ [] THEN DAT = GET_TAG(STR,'DATA')                                                                             ; If "IMAGE" tag is not found, then try the "DATA" tag to extract the data from the structure
          ...
          IF KEYWORD_SET(STACKED_INFILE) THEN BEGIN
            CASE OUTPERIOD OF
              'DOY': STOP ; WHERE(DBSTACKED.PERIOD EQ APERIOD,COUNT)
              'WEEK': DSEQ = WHERE((PERIOD_2STRUCT(DBSTACKED.PERIOD)).WEEK_START EQ PERSTR.WEEK_START,COUNT) 
              'MONTH': DSEQ = WHERE((PERIOD_2STRUCT(DBSTACKED.PERIOD)).MONTH_START EQ PERSTR.MONTH_START,COUNT)                                                                      ; Find the period in the stacked DB database
            ENDCASE
            IF COUNT NE 1 THEN MESSAGE, 'ERROR: Unable to find the period within the output stacked data
            _DATA = DAT[*,*,DSEQ]
          ENDIF ELSE BEGIN
      ...
      ENDIF ELSE BEGIN
        IF KEYWORD_SET(STACKED_TEMP) AND ~FILE_TEST(D3_FILE) THEN BEGIN
          IF N_ELEMENTS(TPFILES) GT 1 THEN MESSAGE, 'ERROR: Need to figure out how to work with more than one file.'
          D3META = D3HASH_METADATA(D3_FILE, DB=D3HASH['FILE_DB'])
          D3HASH['METADATA'] = D3META
          SAVE, D3HASH, FILENAME=D3_FILE, /COMPRESS
          FILE_DELETE, TPFILES[0], /VERBOSE
        ENDIF
      ENDELSE
      D3HASH = []
    ENDFOR ; PERIOD_SETS
