  ...
  FOR S=0, N_ELEMENTS(STATFILES)-1 DO BEGIN
    ...
    FOR R=0, N_ELEMENTS(SPER)-1 DO BEGIN
      ...
      CASE APER.PERIOD_CODE OF
        ...
        'M3': OKPER = WHERE(CPER.PERIOD_CODE EQ 'MONTH3' AND CPER.MONTH_START EQ APER.MONTH_START,/NULL)
        'A': OKPER = WHERE(CPER.PERIOD_CODE EQ 'ANNUAL',/NULL)
        'SEA': OKPER = WHERE(CPER.PERIOD_CODE EQ 'SEASON',/NULL)
      ENDCASE
      ...
      IF N_ELEMENTS(DIMS) EQ 2 THEN ADATA = FLTARR(SPX) ELSE ADATA = FLTARR(SPX,SPY)                               ; Create the output data array
      ADATA[*] = MISSINGS(ADATA)  
      CASE ANOM OF
        'RATIO': ADATA[OK_DATA]=SDT[OK_DATA]/CDT[OK_DATA]
        'DIF':   ADATA[OK_DATA]=SDT[OK_DATA]-CDT[OK_DATA]
      ENDCASE
      ANOMHASH[D3_ANOM,*,*,SEQ] = ADATA
