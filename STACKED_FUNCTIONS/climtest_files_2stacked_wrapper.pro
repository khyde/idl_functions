; $ID:	FILES_2STACKED_WRAPPER.PRO,	2023-09-21-13,	USER-KJWH	$
  PRO CLIMTEST_FILES_2STACKED_WRAPPER, DATASETS, PRODS=PRODS, VERSION=VERSION, MAP_IN=MAP_IN, MAP_OUT=MAP_OUT, L3BSUBMAP=L3BSUBMAP, $
      PERIODS=PERIODS, DATERANGE=DATERANGE, STACKED_TEMP=STACKED_TEMP, TEMP_PERIOD=TEMP_PERIOD, REFRESH=REFRESH, ANALYSIS_ERROR=ANALYSIS_ERROR, OVERWRITE=OVERWRITE
;+
; NAME:
...
;   TEMP_PERIOD..... Input period code for STACKED_TEMP files
;   LOGLUN.......... If provided, the LUN for the log file
;   DATERANGE.... The daterange of the input files
;
; KEYWORD PARAMETERS:
;   STACKED_TEMP.... Passed to FILES_2STACKED -> if set, will create STACKED_TEMP files that will be used when calculating the climatological stats
...
;   Jan 04, 2024 - KJWH: Changed DOY keyword to STACKED_TEMP in order to create similar weekly and monthly STACKED_TEMP files
;                        Added TEMP_PERIOD to input the period code for the input files to the STACKED_TEMP
;-
; ****************************************************************************************************
        ...
        'OCCCI': BEGIN 
          ...
          CASE MAP_IN OF 
            ...
            '4KM': BEGIN & MAPIN='SOURCE_MONTHLY' & DMAP='L3B4' & PERS='MONTHLY' & END
              ELSE: BEGIN & MAPIN='SOURCE' & DMAP='L3B4' & IF PERS[0] EQ '' THEN PERS=['DAILY'] & IF KEYWORD_SET(STACKED_TEMP) THEN PERS = '' & END
           ENDCASE  
        END
  ...
  ;'PHYTO_SIZE-TURNER': BEGIN & NPROD='PHYTO_SIZE-TURNER' & OPRODS=['MICRO','NANO','PICO','FMICRO','FNANO','FPICO'] + '-TURNER' & IF VERSION EQ '5.0' THEN FILETYPE='SAVE' & MAPIN='L3B4' & APER='' & END
          ...
          IF KEYWORD_SET(STACKED_TEMP) THEN BEGIN
            FILETYPE=[]
            IF ~N_ELEMENTS(TEMP_PERIOD) THEN PERIOD = 'DD' ELSE PERIOD = TEMP_PERIOD
            IF TEMP_PERIOD EQ 'DOY' THEN PERIOD = 'DD'
            NPROD = APROD
          ...
          FILES = GET_FILES(DSET, PRODS=NPROD, MAPS=MAPIN, FILE_TYPE=FILETYPE,PERIODS=PERIOD, VERSION=VERSION,DATERANGE=DTR)
          ...
          FILES_2STACKED, FILES, PRODS=OPRODS, STAT_TYPES=STAT_TYPES, D3_FILES=D3_FILES, DIR_OUT=DIR_OUT, MAP_OUT=OMAP, L3BSUBMAP=L3BSUBMAP, FILE_LABEL=FILE_LABEL, $
            MAINPROD=MPROD,DATERANGE=DTR, OUTFILE=OUTFILE, LOGLUN=LOGLUN, STACKED_TEMP=STACKED_TEMP, VERBOSE=VERBOSE, TESTING=TESTING, OVERWRITE=OVERWRITE
