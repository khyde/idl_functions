...
; ===> Get the JD RANGE for the interpolation to standard days
  ...
  CASE PSTR.PERIOD_CODE OF
    ...
    END  
    'MONTH3': BEGIN
      MTS = PERIOD_SETS(JD_GEN([SD[0],DP.DATE_END]),PERIOD_CODE='M3',JD_START=JD_START,JD_END=JD_END)
      SETS = PERIOD_SETS(PERIOD_2JD(MTS.PERIOD),PERIOD_CODE='MONTH3',/NESTED,JD_START=JD_START,JD_END=JD_END)
      PERIOD = TAG_NAMES(SETS)
    END
