; $ID:	D3HASH_PERIOD_SETS.PRO,	2023-09-21-13,	USER-KJWH	$
  FUNCTION D3HASH_PERIOD_SETS, FILES, OUTPERIOD=OUTPERIOD, BY_YEAR=BY_YEAR, CLIMATOLOGY_DATERANGE=CLIMATOLOGY_DATERANGE
...
; ****************************************************************************************************
  ...
  IF ~N_ELEMENTS(CLIMATOLOGY_DATERANGE) THEN CLIMDTR = [] ELSE CLIMDTR = DATE_2YEAR(CLIMATOLOGY_DATERANGE)
...
; ===> Get the date information for the input periods
  ...
  JD_START = DATE_2JD(DATE_RANGE[0])>DATE_2JD(SENSOR_DATERANGE[0]) & JD_END = DATE_2JD(DATE_RANGE[1])                                                   ; Get the julian dates for the first and last dates
...
; ===> Set up output structure by year  
  ...
  ENDIF ELSE BEGIN
    IF KEYWORD_SET(CLIMATOLOGY) THEN BEGIN
      IF N_ELEMENTS(CLIMDTR) NE 2 THEN MESSAGE, 'ERROR: Must provide the climatology daterange'
      SENSOR_YEARS = [PSTR.YEAR_START,PSTR.YEAR_END]
      IF SENSOR_YEARS[0] GT CLIMDTR[1] OR SENSOR_YEARS[1] LT CLIMDTR[0] THEN MESSAGE, 'ERROR: Check the climatology daterange and sesnor daterange' 
      CLIMYRS = NUM2STR([FIX(CLIMDTR[0])>FIX(SENSOR_YEARS[0]),FIX(CLIMDTR[1])<FIX(SENSOR_YEARS[1])])
      CLIMRANGE = GET_DATERANGE(CLIMYRS)
      
      CASE OSTR.PERIOD_CODE OF
        'ANNUAL': BEGIN
          IF N_ELEMENTS(FILES) NE 1 THEN MESSAGE, 'ERROR: Only one input file allowed for "ANNUAL" output files'
          STACKED_PERIOD = 'ANNUAL_' + CLIMYRS[0] + '_' + CLIMYRS[1]
        ...
        'MONTH': BEGIN
          ...
          SET = PERIOD_SETS(JD_GEN(CLIMDTS),PERIOD_CODE='M')  ; Create the monthly periods as an input to the MONTH period
          SET = PERIOD_SETS(PERIOD_2JD(SET.PERIOD), PERIOD_CODE='MONTH') ; 
          STACKED_PERIOD = 'MONTH' + '_00_' + CLIMYRS[0] + '_' + CLIMYRS[1]
        ...
        'WEEK': BEGIN
          ...
          FP = PARSE_IT(FILES) & DP = PERIOD_2STRUCT(FP.PERIOD)
          CLIMDTS = JD_2DATE([DATE_2JD(CLIMRANGE[0])>DATE_2JD(DP.DATE_START),DATE_2JD(CLIMRANGE[1])<DATE_2JD(DP.DATE_END)])
          SET = PERIOD_SETS(JD_GEN(CLIMDTS),PERIOD_CODE='W')  ; Create the monthly periods as an input to the MONTH period
     ;     SET = PERIOD_SETS(JD_GEN([DP.DATE_START,DP.DATE_END]),PERIOD_CODE='W')  ; Create the weekly periods as an input to the WEEK period
          SET = PERIOD_SETS(PERIOD_2JD(SET.PERIOD), PERIOD_CODE='WEEK') ; 
          STACKED_PERIOD = 'WEEK_00_' + CLIMYRS[0] + '_' + CLIMYRS[1]
          YRPER = REPLICATE(CREATE_STRUCT('STACKED_PERIOD','', 'FILENAME',''),N_ELEMENTS(SET))
          YRPER.STACKED_PERIOD = STACKED_PERIOD
          YRPER.FILENAME = FILES
          SETS = STRUCT_MERGE(YRPER,SET)          
        ...
        'DOY': BEGIN
          ...
          ENDIF ELSE BEGIN
            ...
            STACKED_PERIOD =  'DOY_000_' + CLIMYRS[0] + '_' + CLIMYRS[1]
            YRPER = REPLICATE(CREATE_STRUCT('STACKED_PERIOD','', 'FILENAME',''),N_ELEMENTS(SET))
            YRPER.STACKED_PERIOD = STACKED_PERIOD
            SETS = STRUCT_MERGE(YRPER,SET)
    ...
    ENDIF ELSE BEGIN
      ...
      CASE CSTR.STAT_INPUT_PERIOD OF
        'D': SPERS = PSTR.PERIOD
        'W': SPERS = YEAR_WEEK_RANGE(SENSOR_DATERANGE[0],DATE_END,/PERIOD);(SENSOR_DATERANGE[0],SENSOR_DATERANGE[1],/PERIOD)
        'M': SPERS = YEAR_MONTH_RANGE(SENSOR_DATERANGE[0],DATE_END,/PERIOD) ; SENSOR_DATERANGE[0],SENSOR_DATERANGE[1],/PERIOD) ; DATE_START,DATE_END
        'A': SPERS = YEAR_RANGE(SENSOR_DATERANGE[0],DATE_END,PERIOD='A');(SENSOR_DATERANGE[0],SENSOR_DATERANGE[1],PERIOD='A')
        'M3': SPERS = YEAR_MONTH_RANGE(SENSOR_DATERANGE[0],DATE_END,/PERIOD)
      ENDCASE
      
      SET = PERIOD_SETS(PERIOD_2JD(SPERS),PERIOD_CODE=OUTPER,JD_START=JD_START,JD_END=JD_END)                                                             ; Group the output periods based on the input periods and output period code
