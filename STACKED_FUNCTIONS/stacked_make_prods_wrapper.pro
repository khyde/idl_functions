...
; ****************************************************************************************************
  ...
  IF HAS(EXTRA,'VERSION') THEN VERSION = EXTRA.VERSION ELSE VERSION = []
  ...
  FOR D=0, N_ELEMENTS(DATASETS)-1 DO BEGIN
    ...
    FOR M=0, N_ELEMENTS(MAPS)-1 DO BEGIN
      ...
      IF KEYWORD_SET(DO_PSC) THEN BEGIN  
        CF = GET_FILES(DSET,VERSION=VERSION,PRODS=CPROD,FILE_TYPE='STACKED_SAVE',DATERANGE=DR,MAPS=MAPIN)
        SFA = GET_FILES('AVHRR',PRODS='SST', FILE_TYPE='STACKED_INTERP',DATERANGE=['1997','2002'],MAPS=MAPIN)
      ...
      IF KEYWORD_SET(DO_PSC_PPD) THEN BEGIN
        ...
        PPDF = GET_FILES(DSET,PRODS='PPD-VGPM2',VERSION=VERSION,FILE_TYPE='STACKED_STATS', PERIOD='M', DATERANGE=DR)
        PSCF = GET_FILES(DSET,PRODS='PSC_FMICRO-TURNER',VERSION=VERSION,FILE_TYPE='STACKED_STATS', PERIOD='M',DATERANGE=DR)
