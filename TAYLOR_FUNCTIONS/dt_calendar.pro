; $ID:	DT_CALENDAR.PRO,	JUNE 06 2005, 13:38	$
;
...
;	DT_CALENDAR
;	DT_CALENDAR, YEAR
;	DT_CALENDAR, MONTH, YEAR
;
; INPUTS:
;	If called without arguments, DT_CALENDAR draws a calendar for the
;	current month.
;
...
;   JOR, Sept 21, 2003  Copied from rsi: calendar.pro
;		ADDED DOY AS FOLLOWS:
;; 		RSI    xyouts, /norm, size=TSIZE, align = NALIGN, X[COL], Y[ROW], I
;;		Julian = JULDAY(Month, i, Year) & DOY=STRTRIM(FIX(DT_JULIAN2DOY(JULIAN)),2)& TXT=STRTRIM(I,2)+'!C'+DOY
...
DAY1_NEXT_YEAR = (JULDAY(1, 1, (YEAR eq -1) ? 1 : YEAR+1) + 1) MOD 7

; Note special case for Gregorian Reform in 1582.
if (((DAY1_NEXT_YEAR + 7 - DAY) mod 7) eq 1) or (year eq 1582) then $
    MONTHS[1] = 28	; Not a leap year
...
for I = 1, NUM_DAYS do begin
    ...
    if (year eq 1582) then begin
        ; Pope Gregory XIII issues Gregorian Calendar reform.
        ; Thursday, Oct 4, 1582 is followed by Friday, Oct 15.
        if (month eq 10) and (i gt 4) and (i lt 15) then continue
    endif
;; RSI    xyouts, /norm, size=TSIZE, align = NALIGN, X[COL], Y[ROW], I
	Julian = JULDAY(Month, i, Year) & DOY=STRTRIM(FIX(JD_2DOY(JULIAN)),2)& TXT=STRTRIM(I,2)+'!C!D'+DOY(0)+'!N'


	xyouts, /norm, size=TSIZE, align = NALIGN, X[COL], Y[ROW], TXT
    COL = COL + 1
    endfor
