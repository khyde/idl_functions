; $Id: //depot/Release/IDL_81/idl/idldir/lib/resolve_all.pro#1 $
;
...
function resolve_all_body, quiet, cont, do_func, skipr, didIResolve
;
    ...
    if strlen(a[0]) gt 0 then begin
        ...
        for i=0, num-1 do begin
            ...
            if error eq 0 then begin
                ...
                if (~KEYWORD_SET(didIResolve)) then begin
...
PRO resolve_all, CLASS=class, CONTINUE_ON_ERROR=cont, QUIET = quiet, $
    ...
    repeat begin
        cnt = 0
        cnt = cnt + resolve_all_body(quiet, local_cont, 0, skipr, didIResolve)
        cnt = cnt + resolve_all_body(quiet, local_cont, 1, skipr, didIResolve)
