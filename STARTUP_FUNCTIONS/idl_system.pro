...
; ===> Add the basic directory information to the structure
  ...
  SUBDIRS = ['DATASETS_SOURCE','DATASETS','PROJECTS'];,'SCRIPTS']
  TAGS = TAG_NAMES(S)
  FOR N=0, N_ELEMENTS(SUBDIRS)-1 DO BEGIN
    ...
    FOR F=0, N_ELEMENTS(DIRS)-1 DO BEGIN
      ...
      IF SUBDIRS[N] EQ 'DATASETS_SOURCE' THEN BEGIN
        ANAME = ANAME + '_SOURCE'
        S = CREATE_STRUCT(S,ANAME,SPATH+SUBDIRS[N]+SL+DIRS[F])
      ENDIF ELSE S = CREATE_STRUCT(S,ANAME,PATH+SUBDIRS[N]+SL+DIRS[F])
...
; ===> Add the IDL specific directories 
 ...
 ; S = CREATE_STRUCT(S,'PROGRAMS',S.IDL + 'PROGRAMS' + SL) ; 2020-06-03 Will need to include the main programs directory until the transition to the new "project" based organization is complete
  ...
  FOR F=0, N_ELEMENTS(IDLDIRS)-1 DO BEGIN
    ...
    IF NAME EQ 'RESOURCES' OR NAME EQ 'FUNCTIONS' THEN BEGIN
      ...
      FOR G=0, N_ELEMENTS(GDIRS)-1 DO BEGIN
        ...
        S = CREATE_STRUCT(S,GNAME,SUBDIR+NAME+SL+GNAME+SL)
        IF GNAME EQ 'FILES' OR GNAME EQ 'IDL_DATA' OR GNAME EQ 'IDL_TEST' THEN BEGIN
