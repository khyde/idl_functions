; $ID:	SATSHIP_FIND_LONLAT.PRO,	2020-06-30-17,	USER-KJWH	$
;+
; NAME:
;   SATSHIP_FIND_LONLAT.PRO
;
; PURPOSE:
...
;                               Changed name to SATSHIP_FIND_LONLAT to be in the SATSHIP family
...
;-
; ****************************************************************************************************
FUNCTION SATSHIP_FIND_LONLAT, LON, LAT, GLOBAL=GLOBAL, LATITUDE=LATITUDE, LONGITUDE=LONGITUDE, ERROR=ERROR, ERR_MSG=ERR_MSG

  ROUTINE_NAME='SATSHIP_FIND_LONLAT'
  ...
  NSCANS = GLOBAL.NUMBER_OF_SCAN_LINES[0]
  NPIX   = GLOBAL.PIXELS_PER_SCAN_LINE[0]
...
; final implementation, utilizing all IDL array abilities, FASTEST solution
  ...
  DMAX  = MAX(DOTA)
  SOLN  = ARRAY_INDICES(DOTA, WHERE(DOTA EQ DMAX))
  ...
  IF (PIXSN) GT NSCANS-1 THEN FAIL = 1
  IF (PIXPX) GE NPIX-1 THEN FAIL = 1
  ...
  TWO2ONE, PIXPX, PIXSN, LONA, INDEX
  RETURN, INDEX
END
