; $ID:	OC_LIST.PRO,	2020-06-30-17,	USER-KJWH	$

PRO OC_LIST ,DATA, SKIP=skip, NOSEQ=NOSEQ, NOHEADING=noheading, FILE=file,NOTES=notes, DELIM=delim,Tags=Tags,TAGNAMES=Tagnames,LIST_OVERWRITE=list_overwrite
...
; *************************************************************************
  ...
  ROUTINE_NAME='OC_LIST'
  IF N_ELEMENTS(DATA) EQ 0 THEN GOTO, DONE
  ...
  ENDCASE
	...
	ENDIF ELSE BEGIN
		IF N_ELEMENTS(TAGS) GE 1 OR N_ELEMENTS(TAGNAMES) GE 1 THEN BEGIN
			COPY = STRUCT_COPY(DATA,TAGS=TAGS,TAGNAMES=TAGNAMES)
		ENDIF 	ELSE COPY = DATA
...
;FIXME: Big problem here if FILE can't be accessed (eg drive not available), first time run, etc...
...
; 	=====> Print Data
	...
	ENDIF ELSE BEGIN
; =====> Printing to a File
    IF KEYWORD_SET(LIST_OVERWRITE) THEN OPENW,LUN,FILE,/GET_LUN,error=myerror ELSE OPENW,LUN,FILE,/GET_LUN,/APPEND,error=myerror
    if myerror ne 0 then print,!error_state.msg
