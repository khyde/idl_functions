; $ID:	MEDFILTER2D_3X3.PRO,	2020-07-08-15,	USER-KJWH	$

	FUNCTION MEDFILTER2D_3X3, ARRAY, WIDTH=WIDTH, DIFF= diff, NITER=NITER, MAX_ITER = MAX_ITER, ERROR=error, ERR_MSG=err_msg
...
;			Written Dec 1, 2006 by Igor Belkin, URI, GSO.
;-

	ROUTINE_NAME='MEDFILTER2D_3X3'
...
;	===> Default window is 3x3
	IF N_ELEMENTS(WIDTH) NE 1 THEN WIDTH = 3
...
;		LLLLLLLLLLLLLLLLLLLLLLLL
    FOR i=1,Nrows-2 DO BEGIN
;			LLLLLLLLLLLLLLLLLLLLLLLL
			FOR j=1,Ncols-2 DO BEGIN
				W =  _ARRAY(J-1:J+1 , I-1:I+1)
        COPY(J,I)=MEDIAN(W)
      ENDFOR ; j-loop
   	ENDFOR ; i-loop
