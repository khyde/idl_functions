; $ID:	STRUCT_VALUE.PRO,	2020-06-30-17,	USER-KJWH	$
;+
;	This Function returns the unique values in a structure

;
; SYNTAX:
;		Result = STRUCT_VALUE(Struct)
; OUTPUT:
;		Structure
...
;   Result = STRUCT_VALUE(STRUCT,TAGNAMES='AA')   			; Extract Tag AA
;
;
; NOTES:
...
;-
; *************************************************************************
FUNCTION STRUCT_VALUE, Struct,Names, TAGNAMES=Tagnames, TAGS=Tags,   ERROR=error
  ROUTINE_NAME='STRUCT_VALUE'
...
;	===> Eliminate redundant tag_targets
	...
	VAL = REPLICATE(CREATE_STRUCT('NAME','','FIRST','','LAST','','VALUE','','N',''),N_ELEMENTS(TAG_TARGETS))
...
; =====> Make a new structure of values for the TAG_TARGETS
  FOR nth = 0L, N_ELEMENTS(TAG_TARGETS)-1L DO BEGIN
    ...
    aval = Struct[0].(ATAG)
		...
		VAL[NTH].NAME=ANAM
		VAL[NTH].FIRST=FIRST(SETS.VALUE)
		VAL[NTH].LAST =LAST(SETS.VALUE)
