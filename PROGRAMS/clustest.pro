; $ID:	CLUSTEST.PRO,	2020-07-08-15,	USER-KJWH	$
...
END                 ;Procedure Vanova
 ...
 SizeB= dblarr(K)          ;SizeB(i) = # of cases in cluster i
...
Common KBlock,B
 ...
 D=dblarr(K,R)
 INCLUS=dblarr(R,k)
 L=dblarr(K)                      ;L(i)= size of ith cluster
...
Pro clustest,Data1,CLuster,VarName = VarName,CaseName = CaseName,$
...
;on_error,2
 ...
 if(N_Elements(nr) NE 0) THEN  if ( nr EQ 1) THEN BEGIN
  ...
  Cluster = dblarr(R)    ; Clus(i)= cluster containing case i
