; $ID:	GET_TAG_DEMO.PRO,	2020-07-01-12,	USER-KJWH	$
;+
; ######################################################################### 
  PRO GET_TAG_DEMO

;  PURPOSE:  GET_TAG_DEMO IS A DEMO FOR THE GET_TAG FUNCTION
...
;     NOV 15,2015,JOR UPDATED TO REFLECT CHANGES TO GET_TAG
;-
; #########################################################################

;*****************************
ROUTINE_NAME  = 'GET_TAG_DEMO'
;*****************************
...
;*************************
  , 'DO_NC'
  PRINT,'THIS STEP SHOWS HOW GET_TAG MAY GET THE IMAGE DATA IN THE SD FROM A NC FILE'
  ...
  FOR NTH = 0,NOF(FILES)-1 DO BEGIN
     ...
     IF HAS_TAG(D,'SD.CHLOR_A.IMAGE') THEN DATA = GET_TAG(D,'SD.CHLOR_A.IMAGE')
...
IF KEY(DO_MAPS_DB) THEN BEGIN
 ;****************************
  , 'DO_MAPS_DB'
  PRINT,'THIS STEP SHOWS HOW GET_TAG MAY GET THE DATA FROM THE MAPS MASTER DATABASE' & WAIT,1
  DB = MAPS_READ()
...
IF KEY(DO_GET_IMAGE_FROM_SD) THEN BEGIN
  ...
  PRINT,'THIS STEP SHOWS HOW GET_TAG MAY GET THE IMAGE FROM FROM A SAVE MADE WITH STRUCT_SD_WRITE' & WAIT,1
  ...
  FOR NTH = 0,NOF(FILES)-1 DO BEGIN
    ...
    DATA = GET_TAG(STRUCT,'CHLOR_A.IMAGE')
