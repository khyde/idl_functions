...
FUNCTION MAPS_L3B_LONLAT_2BIN, L3BMAP, LONS, LATS, BASEBIN=BASEBIN, NUMBIN=NUMBIN, LATBIN=LATBIN, TOTBINS=TOTBINS
...
;     JUL 29, 2019 - KJWH: Now using MAPS_BIN2LL to get the BASEBIN and NUMBIN information                     
;-
; |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
; ******************************  
  ...
  B = MAPS_BIN2LL(L3BMAP, /SKIP_LONLAT)
	BASEBIN = B.FIRST_BIN
	NROWS = B.NROWS
  NUMBIN = B.NBINS_ROW
...
; Get idx number for specific lats & lons
    ...
    IF ~FINITE(LAT) OR ~FINITE(LON) THEN CONTINUE
		...
		ROW = ROW < NROWS
		; ROW = ROW < NROWS-1 When ROW equals NROWS, may need to subtract 1 if NUMBIN[ROW] throws an error
