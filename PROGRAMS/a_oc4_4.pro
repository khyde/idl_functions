; $ID:	A_OC4_4.PRO,	2020-07-08-15,	USER-KJWH	$


 FUNCTION  A_OC4_4 , Rrs443,Rrs490, Rrs510, Rrs555, NOTES=NOTES, LWN=LWN, COEFFS=coeffs, WLS=wls
;+
; NAME:
;       A_OC4_4  'Ocean Chlorophyll 4 version 4'
;
; PURPOSE:
...
;
; CALLING SEQUENCE:
;       C = A_OC4_4(Rrs443,Rrs490,Rrs510,Rrs555)
;
; INPUTS:
...
;				Aug 27,2001  T.Ducas,   AND changed to OR in the following:(Nlw443 GT 0d Rrs490 GT 0d OR Nlw510 GT 0d )
;-
...
; Equation Coefficients:
  a = [0.366,-3.067,1.930,0.649,-1.532];  March 30,2000

  IF KEYWORD_SET(COEFFS) THEN RETURN, A
  IF KEYWORD_SET(WLS) 		THEN RETURN,[443,489,510,555]
...
; Whichever (Rrs443,Rrs490,Rrs510) is greatest, ends up in the numerator of the maximum band ratio
  ...
  R = ALOG10((_RRS443[OK] > _RRS490[OK] > _RRS510[OK]) / _RRS555[OK])

; ====================>
; Calculate chlorophyll concentration (ug/l)
; july 24,1999  C = 10.0^(a[0] + a[1]*R +  a(2)*R^2   + a(3)*R^3)  +a(4)
