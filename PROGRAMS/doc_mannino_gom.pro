; $ID:	DOC_MANNINO_GOM.PRO,	2020-07-08-15,	USER-KJWH	$

 FUNCTION DOC_MANNINO_GOM, A_CDOM_355=A_CDOM_355, DATE=DATE,  REFRESH=refresh, ERROR=ERROR
...
; CALLING SEQUENCE:
;
;		Result = DOC_MANNINO( A_CDOM_355=A_CDOM_355, DATE=DATE)
;
; INPUTS:
...
;
;	PROCEDURE:
;		This algorithm for the Gulf of Maine uses two seasonal functions relating ACDO at 355nm to Dissolved Organic Carbon (micromolar):
...
;	********************************************************************************************
  ...
  IF N_ELEMENTS(A_CDOM_355) LT 1 OR  N_ELEMENTS(DATE) NE 1 THEN BEGIN
    ERROR ='ERROR: Must imput A_CDOM_355 and DATE'
    RETURN,''
  ENDIF
;  ===> Convert input dates to DOY
  IF N_ELEMENTS(DATE) EQ 1 THEN DOY = REPLICATE(FIX(DATE_2DOY(DATE)),N_ELEMENTS(A_CDOM_355)) ELSE DOY = FIX(DATE_2DOY(DATE)) 
...
;	===> Compute DOC according to Winter and Summer Functions
	DOC_WINTER = 1/ ( ALOG(DOUBLE(A_CDOM_355[OK]))*(-0.00328081) + 0.0106521)
	DOC_SUMMER = 1/ ( ALOG(DOUBLE(A_CDOM_355[OK]))*(-0.00354386) + 0.0100959)
