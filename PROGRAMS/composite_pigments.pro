; $ID:	COMPOSITE_PIGMENTS.PRO,	2021-04-15-17,	USER-KJWH	$

PRO COMPOSITE_PIGMENTS, FILES, MP=MP, DIR_OUT=DIR_OUT, OUTFILE=OUTFILE, DATERANGE=DATERANGE, YEARS=YEARS, MONTHS=MONTHS,YEARS_ON_TOP=YEARS_ON_TOP,$
...
;
; PURPOSE:
;   This procedure creates a composite of the pigment data derived from PIGMENTS_PAN
;
; CATEGORY:
;   PLOTTING
;
...
;     Written:  Feb 01, 2019 by K.J.W. Hyde, NOAA/NEFSC/Narragansett Laboratory, 28 Tarzwell Drive, 02882 (kimberly.hyde@noaa.gov)
;     Modified: Feb 04, 2019 - KJWH: Updated the FILE_MAKE to be AFILE instead of FILES
;               Feb 07, 2018 - KJWH: Changed name to COMPOSITE_PIGMENTS to be consistent with other composite programs and easier to find
;               
;               
;-
; ****************************************************************************************************
  ROUTINE_NAME = 'PIGMENTS_COMPOSITE'
  SL = PATH_SEP()
  
  PIGMENTS = ['CHLA','CHLB','CHLC','CARO','ALLO','FUCO','PERID','NEO','VIOLA','DIA','LUT','ZEA']
  PROD = 'PIGMENTS_0.001_1'
  ...
  IF NONE(NCOLS)  THEN NCOLS  = 3
  IF NONE(NROWS)  THEN NROWS  = 4
  ...
  FOR NTH=0, N_ELEMENTS(FILES)-1 DO BEGIN
    ...
    IF FP.PROD NE 'PIGMENTS' THEN CONTINUE ; THIS COMPOSITING ROUTINE CURRENTLY ONLY WORKS WITH THE COMBINED DAILY PIGMENTS FILES 
    ...
    FOR G=0, N_ELEMENTS(PIGMENTS)-1 DO BEGIN
      DAT = GET(S,PIGMENTS(G))
      ...
      TXT = TEXT(XPOS+SPACE/2,YPOS-SPACE*2,PIGMENTS(G),FONT_SIZE=CFONT_SIZE,/DEVICE)
    ENDFOR ; PIGMENTS
