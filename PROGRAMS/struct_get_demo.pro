; $ID:	STRUCT_GET_DEMO.PRO,	2020-07-01-12,	USER-KJWH	$
;+
; ######################################################################### 
  PRO STRUCT_GET_DEMO

;  PURPOSE:  STRUCT_GET_DEMO IS A DEMO FOR THE STRUCT_GET FUNCTION
...
;     NOV 15,2015,JOR UPDATED TO REFLECT CHANGES TO STRUCT_GET
;-
; #########################################################################

;********************************
ROUTINE_NAME  = 'STRUCT_GET_DEMO'
;********************************
...
;*************************
  , 'DO_NC'
  PRINT,'THIS STEP SHOWS HOW STRUCT_GET MAY GET THE IMAGE DATA IN THE SD FROM A NC FILE'
...
IF KEY(DO_MAPS_DB) THEN BEGIN
 ;****************************
  , 'DO_MAPS_DB'
  PRINT,'THIS STEP SHOWS HOW STRUCT_GET MAY GET THE DATA FROM THE MAPS MASTER DATABASE' & WAIT,1
  DB = MAPS_READ()
...
IF KEY(DO_GET_IMAGE_FROM_SD) THEN BEGIN
  ...
  PRINT,'THIS STEP SHOWS HOW STRUCT_GET MAY GET THE IMAGE FROM FROM A SAVE MADE WITH STRUCT_SD_WRITE' & WAIT,1
  ...
  FOR NTH = 0,NOF(FILES)-1 DO BEGIN
    ...
    DATA = STRUCT_GET(STRUCT,'CHLOR_A.IMAGE')
