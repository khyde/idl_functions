; $ID:	STATS_TREND.PRO,	2020-07-08-15,	USER-KJWH	$
;+
;#############################################################################################################
	FUNCTION STATS_TREND,JD,DATA,DATE_RANGE=DATE_RANGE,VERBOSE=VERBOSE,ALPHA = ALPHA,_EXTRA=_EXTRA
...
;      ALPHA: PROB VALUE [DEFAULT = 0.05]
;      DATE_RANGE: ARRAY OF 2 DATES [REQUIRED]
;      _EXTRA: ANY ITEMS IN _EXTRA ARE AUTOMATICALLY ADDED TO THE OUTPUT STRUCTURE

; OUTPUTS:  A STRUCTURE WITH STATS FROM THE REGRESSION RESULTS USING YEARLY VALUES
;           AND YEARLY ANOMALIES
;		
; EXAMPLES: SEE STATS_TREND_DEMO
; 
;
...
;     SEP 3,2014,JOR CHANGED TO A FUNCTION ADDED _EXTRA
;       IF ROUNDS(MEAN_WHERE/MEAN_STATS_PERIOD,4,/SIG) NE 1.0 THEN BEGIN
;     OCT 9,2014,JOR ACCOMMODATING CHANGES TO STATS_PERIOD:
;                    
...
PER_Y     =STATS_PERIOD(JD,DATA,  PERIOD_CODE='Y')
...
IF COUNT GE 1 THEN BEGIN
  ...
  S_Y = STATS2(X,Y,MODEL = 'LSY',ALPHA = ALPHA,/QUIET)
  PRED =  S_Y.INT + X*S_Y.SLOPE 
...
PER_M     =STATS_PERIOD(JD,DATA,  PERIOD_CODE='M')
;IF KEY(VERBOSE) THEN PN,PER_M,' MONTHS IN PER_M'
; ===> M > MONTH
PER_MONTH =STATS_PERIOD(PER_M.JD,PER_M.MEAN,        PERIOD_CODE='MONTH')
...
FOR M = 1,12 DO BEGIN
  ...
  IF FINITE(MEAN_WHERE) AND FINITE(MEAN_STATS_PERIOD) AND ROUNDS(MEAN_WHERE/MEAN_STATS_PERIOD,4,/SIG) NE 1.0 THEN BEGIN
...
PER_Y_ANOM     =STATS_PERIOD(JD_ANOM_STATS_PERIOD,ANOM_STATS_PERIOD,  PERIOD_CODE='Y')
...
S_Y_ANOM=STATS2(X,Y,MODEL = 'LSY',ALPHA=ALPHA)
;|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

;#########################################################
;EXTRACT INFO FROM  S_YEAR AND S_Y_ANOM
S = CREATE_STRUCT($
                  'DATE',S_Y.DATE,$
                  'MODEL',S_Y.MODEL,$
                  'ALPHA',ALPHA,$
                  ...
                  'Y_RSQ',S_Y.RSQ ,$
                  'Y_MEAN',S_Y.MEAN_Y ,$
                  'Y_SLOPE_T_VAL',S_Y.SLOPE_T_VAL ,$
                  'Y_SLOPE_SIG',S_Y.SLOPE_SIG,$
                  'PCT_CHANGE',PCT_CHANGE,$
                  'Y_ANOM_N',S_Y_ANOM.N,$
                  'Y_ANOM_INT',S_Y_ANOM.INT,$
                  'Y_ANOM_SLOPE',S_Y_ANOM.SLOPE,$
                  'Y_ANOM_RSQ',S_Y_ANOM.RSQ ,$
                  'Y_ANOM_MEAN',S_Y_ANOM.MEAN_Y ,$                  
                  'Y_ANOM_SLOPE_T_VAL',S_Y_ANOM.SLOPE_T_VAL ,$                  
                  'Y_ANOM_SLOPE_SIG',S_Y_ANOM.SLOPE_SIG)
;|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||                  

;#############################################3
IF KEY(_EXTRA) THEN BEGIN
  ...
  FOR NTH = 0, N_ELEMENTS(NAMES)-1 DO BEGIN
    TAG = NAMES[NTH]
    VAL = _EXTRA.(NTH)    
    S = CREATE_STRUCT(S,TAG,VAL)    
  ENDFOR;FOR NTH = 0, N_ELEMENTS(_EXTRA)-1 DO BEGIN
  ;FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF  
ENDIF;IF KEY(_EXTRA) THEN BEGIN
;|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
