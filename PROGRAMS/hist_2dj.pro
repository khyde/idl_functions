; $Id:	hist_2dj.pro,	November 26 2006	$
;
; Copyright (c) 1992-2006, Research Systems, Inc.  All rights reserved.
;   Unauthorized reproduction prohibited.
;+
; NAME:
;   HIST_2DJ : Identical to IDL_63_RELEASE OF HIST_2D with changes indicated below ($$$)
;
; PURPOSE:
...
;		J.O'Reilly, NOAA, Narragansett, RI, November, 2006
;		$$$ ADDING 1 BIN IS NOT NECESSARY or desirable when (max1-min1)/b1 is a whole number, same for (max2-min2)/b2
;-
function hist_2dJ, im1, im2, $
...
;		$$$ CHANGES to this routine by J.O'Reilly, NOAA, Narragansett, RI
;		$$$ ADDING 1 BIN IS NOT NECESSARY when (max1-min1)/b1 is a whole number, same for (max2-min2)/b2
    ;Get # of bins for each
;    im1bins = FLOOR((max1-min1) / b1) + 1L
;    im2bins = FLOOR((max2-min2) / b2) + 1L

    ;Get # of bins for each (Add an extra bin only when e.g ((max1-min1) / b1) is not a whole number)
     im1bins = FLOOR((max1-min1) / b1) + 1L*(((max1-min1) / b1) MOD 1 NE 0)
     im2bins = FLOOR((max2-min2) / b2) + 1L*(((max2-min2) / b2) MOD 1 NE 0)
