...
;	  pdf, x, /BANDWIDTH, /SCORE
;
; MODIFICATION HISTORY:
; 	Written by:	Daithi A. Stone (stoned@uvic.ca), 2001-05-09
...
if bandwidthopt then begin
  ...
  for i = 0, nxdata-1 do begin
    xval[i] = where( abs( x[i]-xid ) eq min( abs( x[i]-xid ) ) )
  endfor
  ...
  if dim eq 1 then begin
    ...
    for i = 0, nxdata-1 do begin
      yval[i] = where( abs( y[i]-yid ) eq min( abs( y[i]-yid ) ) )
    endfor
  endelse
...
;      sumpdfcv = sumpdfcv + sum( 1. / eta[idvec]^dim $
;                                 * exp( -0.5 * ( ( x[i] - x[idvec] )^2 $
;                                                   + ( y[i] - y[idvec] )^2 ) $
;                                        / ( eta[idvec] * hvec[h] )^2 ) )

 sumpdfcv = sumpdfcv + TOTAL( 1. / eta[idvec]^dim $
                                 * exp( -0.5 * ( ( x[i] - x[idvec] )^2 $
                                                   + ( y[i] - y[idvec] )^2 ) $
                                        / ( eta[idvec] * hvec[h] )^2 ) )
  ...
  if ( id eq 0 ) or ( id eq nscore-1 ) then begin
    print, 'WARNING:  Smoothing parameter (' + strING(h0) $
...
if not( noplotopt ) then begin
  ...
  if dim eq 2 then begin
    ...
    if keyword_set(fillopt) then c_colors = indgen( nlevels ) + 2 $
                            else c_colors = !p.color + 0 * intarr( nlevels )
  endif
